...
function Get-SmokeStartupFailureDiagnostics {
    ...
    $backendCount = $null
    try {
        $backendCount = @(Get-ManagedBackendProcesses | Where-Object {
            $BaselinePids -notcontains [int]$_.ProcessId
        }).Count
    } catch {}
    ...
    return [pscustomobject]@{
        ...
        managedBackendCount = $backendCount
        logs = $logs
    }
}
...
try {
    ...
    $startupDiagnostics = Get-SmokeStartupFailureDiagnostics `
        -RuntimeDataDir $DataDir -AppProcess $app -BaselinePids $baseline -Token $SessionToken
