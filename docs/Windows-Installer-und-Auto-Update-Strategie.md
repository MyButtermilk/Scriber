...
1. **Inno Setup**, per-user Install nach `%LocalAppData%\Scriber`.
   ...
   - Stabiler `AppId` im Format `{GUID}` fuer saubere Upgrade-Erkennung.
    ...
    - Status 2026-06-09: Ein neuer Standard-NSIS-Build mit `scripts\build_windows.ps1 -SkipChecks -SkipSmoke -ReuseSidecarIfUnchanged -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` ist erfolgreich durchgelaufen. `release-metadata\size-report.json` meldet den Installer mit `186.40 MiB` unter dem `220 MiB` Budget und den Backend-Resource-Tree mit `515.57 MiB`; `runtime-dependency-footprint.json` meldet keine Budget-Failures, keine fehlenden Required Paths und keine disallowed Paths. `media-preparation-smoke.json` meldet `5/5` bestandene Media-Checks. `build-timing.json` meldet `590451 ms` Gesamtzeit; der konkrete NSIS-Lauf war ein Sidecar-Cache-Miss, weil der Tauri-Build vorher neue Frontend-Artefakte erzeugt hatte.
