...
1. **Inno Setup**, per-user Install nach `%LocalAppData%\Scriber`.
   ...
   - Stabiler `AppId` im Format `{GUID}` fuer saubere Upgrade-Erkennung.
    ...
    - Status 2026-06-02: `scripts/smoke_windows_installer.ps1` installiert das erzeugte Setup temporaer, startet die installierte App ohne Dev-Fallback, verifiziert `tauri-supervised` Sidecar-Start und entfernt Testinstallation/Testdaten wieder. Mit `-VerifyFrontend` prueft der Smoke Backend-Static-Assets, Tauri-Origin-CORS fuer `/api/health` und tokenisiertes `/api/runtime` sowie den tokenisierten `/api/runtime/frontend-ready` Beacon aus der echten Tauri-WebView. Mit `-SimulateBackendCrash` toetet der Smoke den Worker, wartet auf den Tauri/Frontend-Recovery-Pfad und prueft `backend-crash-metadata.jsonl`. Mit `-OccupyDefaultPort` belegt er `127.0.0.1:8765` vor App-Start und prueft die dynamische Backend-Portwahl. Mit `-SimulateBackendShutdown` ruft er den token-geschuetzten `/api/runtime/shutdown`-Endpunkt auf und prueft kontrolliertes Worker-Beenden plus Supervisor-Recovery. Mit `-AttachExternalBackend` startet er ein externes Python-Backend und prueft, dass die installierte Tauri-Shell andockt, ohne einen Sidecar zu starten. Mit `-SimulateBackendStartupTimeout` blockiert er den ersten Worker-Start vor Readiness und prueft Supervisor-Ersatzstart. Mit `-StabilityDurationSec <Sekunden>` prueft er wiederholte Health-/State-Probes, stabile Backend-PID, Backend-Working-Set-Samples und normalisierte Tauri/Backend-CPU-Samples vor dem Cleanup; `-MaxBackendWorkingSetGrowthMB <MB>` macht daraus ein Memory-Growth-Gate, `-MaxIdleCpuPercent <Prozent>` ein Average-Idle-CPU-Gate. Mit `-LegacyDataDir <alter Scriber-Pfad> -VerifyLegacyDataMigration -SimulateUpgrade` prueft er Legacy-Datenmigration und Datenerhalt ueber einen zweiten Installerlauf. Mit `-VerifyUninstall` wird der stille Uninstaller zum harten Gate: Uninstaller vorhanden, Exit-Code erfolgreich, Installationsartefakte entfernt, Runtime-Daten-Sentinel erhalten.
    - Status 2026-06-09: Vor der Gyan-Essentials-Umstellung lief ein Standard-NSIS-Build mit `scripts\build_windows.ps1 -SkipChecks -SkipSmoke -ReuseSidecarIfUnchanged -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` erfolgreich durch. Anschliessende `-FastLocalInstaller`-Realbuilds bestaetigten Frontend-Typecheck, Tauri/NSIS-Bundle, Media-Preparation-Smoke, Runtime-Dependency-Footprint und Size-Report im schnellen lokalen Modus. Dieser Full-FFmpeg-Build meldete den Installer mit `186.41 MiB` unter dem `220 MiB` Budget und den Backend-Resource-Tree mit `515.56 MiB`; `runtime-dependency-footprint.json` meldete keine Budget-Failures, keine fehlenden Required Paths und keine disallowed Paths. Nach der Gyan-Essentials-Umstellung bestand `scripts\build_windows.ps1 -FastLocalInstaller -MaxBackendRuntimeDependencyMB 500 -MaxMediaToolsRuntimeDependencyMB 210 -MaxInstallerSizeMB 190` mit Sidecar-Cache-Hit, Installer `152.98 MiB`, Backend `441.88 MiB`, Media-Tools `193.32 MiB`, Release-Media-Preparation-Smoke `5/5` und Runtime-Footprint-Budgets Backend `500 MiB` plus Media-Tools `210 MiB`. Ein installierter Smoke gegen das Gyan-Essentials-Setup bestand mit Frontend/WebView-ready, installierten Media-Tools `5/5`, Support-Bundle-Redaction, striktem Silent-Uninstall und `454.75 MiB` installierter App-Groesse unter dem `500 MiB` Smoke-Budget. Danach bestand Profile B mit aktuellem Installer `102.98 MiB`, installierter App `267.28 MiB`, Backend `254.42 MiB`, Media-Tools `5.84 MiB`, installiertem Frontend-Smoke, installiertem Media-Smoke `5/5` und realem installiertem File-/YouTube-Workflow-Smoke `2/2`; der GitHub-Release-Workflow, FastLocal und direkte lokale Tauri-Builds nutzen Profile B jetzt als Standard. `scripts\smoke_frontend_browser.py` bestand zusaetzlich mit YouTube-Suche-/URL-Thumbnails, File-Drag-and-Drop, Debug-Clear und YouTube-Processing-Detailpfad.
...
1. **Default-Installer = voller Standard-Build fuer Cloud-/Online-Workflows**:
   ...
   - Status 2026-06-09: Der direkte lokale Tauri-Build nutzt wie Release und FastLocal jetzt Profile B als Standard. `Frontend/src-tauri/tauri.conf.json` ruft den Sidecar-Build mit `-UseProfileBFfmpeg -ValidateSlimMediaTools -ReuseSidecarIfUnchanged` auf; vorhandene Profile-B-Binaries werden aus `build\ffmpeg-profile-b-msys2\` wiederverwendet oder über MSYS2/UCRT64 neu gebaut. `scripts/prepare_gyan_ffmpeg_essentials.ps1` bleibt ein expliziter Fallback/Rollback-Pfad und liefert bei Bedarf einen validierten Gyan-`MediaToolsDir`.
   - Status 2026-06-03: `scripts/build_tauri_backend_sidecar.ps1` und `scripts/build_windows.ps1` bieten `-SkipBundledFfprobe` als expliziten Groessenvergleichsmodus. Der vor der SciPy-Entfernung gemessene Vergleichsbuild reduzierte den Installer von 205.79 MiB auf 163.43 MiB und die installierte App auf 481.10 MiB, weil `ffprobe.exe` mit 133.43 MiB aus dem Backend-Resource-Ordner entfaellt. Dieser Modus darf trotzdem nicht als Standard gelten, bis YouTube-/Datei-/Azure-MAI-Media-Smokes den reduzierten Umfang belegen.
   ...
   - Status 2026-06-03: `scripts/smoke_media_preparation.py` prueft zusaetzlich die echten Python-Helfer fuer Datei-Kompression, Upload-Audioextraktion, YouTube-Post-Download-Normalisierung, Azure-MAI-MP3-Vorbereitung und optional `ffprobe`-Dauerprobe gegen die aufgeloesten oder gebuendelten Media-Tools. `scripts/build_windows.ps1 -RunMediaPreparationSmoke` fuehrt diesen Gate gegen `target\release\backend\tools\ffmpeg` aus und schreibt `release-metadata\media-preparation-smoke.json`. `scripts/build_windows.ps1 -RunInstallerMediaPreparationSmoke` installiert danach das NSIS-Paket und fuehrt denselben Helper-Gate gegen die tatsaechlich installierten `backend\tools\ffmpeg`-Binaries aus.
...
3. **Vorteil**:
   ...
   - Groessenoptimierung darf ueber schlankere kompatible Media-Binaries, Dependency-Cleanup oder explizite Vergleichsbuilds wie `-SkipBundledFfprobe` erfolgen, aber nicht durch stilles Entfernen von Media-Tools aus dem Standard-Build. Schlankere FFmpeg-Binaries muessen mindestens `-ValidateSlimMediaTools`, `smoke_media_preparation.py` gegen Release- und installierten App-Ordner sowie danach reale installierte YouTube-/Datei-/Azure-MAI-Smokes liefern. Fuer Profile B ist dieser Gate-Stand am 2026-06-09 erreicht.
...
6. Status 2026-06-09: `scripts/analyze_backend_runtime_dependencies.py` misst und gated die gefrorenen SciPy-/ONNXRuntime-Anteile sowie Standard-Release-Komponenten im Sidecar. Der aktuelle Release-Backend-Snapshot meldet `33.75 MiB` fuer die SciPy/ONNXRuntime-Gruppe: `0.00 MiB` fuer `scipy` + `scipy.libs` und `33.75 MiB` fuer `onnxruntime`. Zusaetzlich reportet der Gate Component-Footprints fuer den kompletten Backend-Sidecar, `_internal`, `tools\ffmpeg`, PySide6, Google/gRPC und Pillow. Der Report erwartet SciPy als absent, prueft ONNXRuntime/Silero-VAD-Pflichtpfade, lehnt ONNXRuntime-Beispiel-/Tooldaten sowie ungenutzte Pillow-AVIF-Binaries ab und kann Component-Budgets hart failen.
...
1. In `scriber.spec` nur benoetigte `hiddenimports`/`datas` aufnehmen.
   ...
   - Status 2026-06-09: `scripts/build_windows.ps1 -RunRuntimeDependencyFootprint` schreibt `release-metadata\runtime-dependency-footprint.json`; optionale Budgets `-MaxScipyRuntimeDependencyMB`, `-MaxOnnxRuntimeDependencyMB`, `-MaxPythonRuntimeDependencyMB`, `-MaxBackendRuntimeDependencyMB`, `-MaxInternalRuntimeDependencyMB`, `-MaxMediaToolsRuntimeDependencyMB`, `-MaxPySide6RuntimeDependencyMB`, `-MaxGoogleGrpcRuntimeDependencyMB` und `-MaxPillowRuntimeDependencyMB` machen Groessenregressionen hart.
...
3. Jede Release-PR enthaelt den Groessenvergleich zur Vorversion.
   - Status 2026-06-09: `scripts/create_release_size_report.py` erzeugt `size-report.json` mit Artefaktgroessen, Top-Dateien und optionaler installierter App-Groesse. `scripts/build_windows.ps1` ruft den Report nach `latest.json`/`SHA256SUMS.txt` auf und failt standardmaessig, wenn das groesste Installer-Artefakt ueber `-MaxInstallerSizeMB 220` liegt. `scripts/build_windows.ps1` schreibt zusaetzlich `release-metadata\build-timing.json`; der Sidecar-Build schreibt `sidecar-build-metadata.json`, das Cache-Status, Phasenzeiten, vorbereitete Media-Tools und PySide6-Pruning-Evidenz enthaelt. `scripts/smoke_windows_installer.ps1 -MaxInstalledSizeMB <MB>` misst den temporaeren Installationsordner und kann die installierte Groesse als hartes Gate pruefen. Der GitHub-Workflow kopiert `release-metadata\*.json`, sodass `size-report.json`, `build-timing.json`, `media-preparation-smoke.json` und `runtime-dependency-footprint.json` als Release-Artefakte verfuegbar sind. Der letzte Standard-Full-NSIS-Setup lag mit `186.41 MiB` unter dem 220-MiB-Gate. Der Standard-Backend-Resource-Ordner lag nach Pillow-AVIF-Pruning bei `515.56 MiB`; die alten Full-Build-`ffmpeg.exe` und `ffprobe.exe` waren mit jeweils rund `133 MiB` die groessten Bloecke. Nach der Gyan-Essentials-Umstellung misst der Fast-Local-NSIS-Installer `152.98 MiB`, der Tauri-Release-Backend-Ordner `441.88 MiB`, das Media-Tools-Paar `193.32 MiB` und die installierte App `454.75 MiB`. Mit aktuellem Profile-B-Standard misst der Installer `102.98 MiB`, die installierte App `267.28 MiB`, der Backend-Resource-Ordner `254.42 MiB` und `tools\ffmpeg` `5.84 MiB`.
...
2. **Build-Pipeline**:
   ...
   - Status 2026-06-02: Authenticode-Validierung ist als optionales Release-Gate umgesetzt. `scripts\validate_windows_authenticode.ps1` prueft gueltige Authenticode-Signaturen, optional erwarteten Publisher und optional Timestamp und kann mit `-OutputPath` einen JSON-Report schreiben. `scripts\build_windows.ps1 -RequireAuthenticodeSignature` ruft das Gate fuer erzeugte `.exe`/`.msi` Artefakte auf und schreibt `release-metadata\authenticode.json`; `.github/workflows/release-windows.yml` kann es mit `SCRIBER_REQUIRE_AUTHENTICODE_SIGNATURE=1` aktivieren und sammelt den JSON-Report mit den Release-Artefakten ein.
...
| `scripts/build_tauri_backend_sidecar.ps1` | Umgesetzt: Import-Preflight -> optionaler Frontend Build -> optionaler Sidecar-Hash-Cache -> PyInstaller Sidecar -> optionales PySide6-Pruning-Experiment -> gefrorener Runtime-Import-Check -> optionales FFmpeg/FFprobe-Bundling -> `sidecar-build-metadata.json` -> optionaler Copy nach Tauri Release. |
| `scripts/build_windows.ps1` | Umgesetzt fuer Tests -> Tauri/NSIS Bundle -> Release-Metadaten -> Manifest-zu-Artefakt-Integritaetsgate -> Release-Groessenreport -> Build-Timing-Report -> Release-/Installer-Smoke-Test. `-FastLocalInstaller` ist der explizite schnelle lokale Installer-Modus: Sidecar-Cache an, vollstaendige Python-Tests und Tauri-Release-Smoke aus, Frontend-Typecheck/Build sowie Media-Preparation- und Runtime-Footprint-Gates mit Standardbudgets bleiben an. `-MaxInstallerSizeMB` aktiviert das Standard-Setup-Groessenbudget von 220 MiB; `-InstallerMaxInstalledSizeMB` reicht ein installiertes App-Groessenbudget an den Installer-Smoke weiter; `-RunRuntimeDependencyFootprint` kann Backend-, `_internal`-, Media-Tools-, PySide6-, Google/gRPC- und Pillow-Budgets hart failen; `-ReuseSidecarIfUnchanged` aktiviert nur den lokalen Sidecar-Cache; `-MediaToolsDir` erlaubt explizite Slim-Media-Kandidaten; PySide6-Pruning-Schalter bleiben Experimente bis installierte Overlay-Smokes bestanden sind. `-RequireAuthenticodeSignature` aktiviert das Windows-Signatur-Gate nach einem externen Signing-Schritt; `-ExpectedAuthenticodePublisher` und `-RequireAuthenticodeTimestamp` verschaerfen es. `-RunInstallerCrashSmoke` fuehrt den installierten Worker-Crash-Recovery-Gate aus; `-RunInstallerPortConflictSmoke` prueft dynamische Backend-Portwahl bei belegtem `127.0.0.1:8765`; `-RunInstallerControlledShutdownSmoke` prueft den token-geschuetzten kontrollierten Worker-Shutdown mit Supervisor-Recovery; `-RunInstallerExternalBackendSmoke` prueft External-Backend-Attach ohne Sidecar-Spawn; `-RunInstallerStartupTimeoutSmoke` prueft Supervisor-Ersatzstart nach nicht-ready Worker; `-RunInstallerFrontendSmoke` prueft installierte Frontend-Entrypoint-/JS-/CSS-Auslieferung ueber den laufenden Backend-Static-Fallback plus Tauri-Origin-CORS fuer `/api/health`, tokenisiertes `/api/runtime` und den echten WebView-Ready-Beacon; `-RunInstallerStabilitySmoke` prueft wiederholte Health-/State-Probes, stabile Backend-PID, Speicher- und CPU-Samples; `-InstallerMaxBackendWorkingSetGrowthMB` aktiviert ein Memory-Growth-Gate fuer lange Stability-Laeufe; `-InstallerMaxIdleCpuPercent` aktiviert ein Average-Idle-CPU-Gate; `-RunInstallerLegacyDataSmoke -RunInstallerUpgradeSmoke` prueft Legacy-Datenmigration und Datenerhalt ueber einen zweiten Installerlauf; `-RunInstallerUninstallSmoke` prueft die stille Deinstallation als striktes Gate. Zusaetzlich optional `-EnableTauriUpdater` fuer signierte Tauri-Updater-Artefakte und Manifest-Signatur-Gates. Der eigentliche Authenticode-Signing-Schritt mit Zertifikat/Cloud-Signing-Provider bleibt offen. |
| `scripts/smoke_windows_installer.ps1` | Umgesetzt: installiert das NSIS-Artefakt temporaer, prueft den installierten Tauri/Sidecar-Start ohne Python/Node-Dev-Fallback, kann mit `-VerifyFrontend` den Backend-Start plus Frontend-Entrypoint, referenzierte JS/CSS-Assets, Tauri-Origin-CORS und den echten WebView-zu-Backend-Ready-Beacon per HTTP 200/nicht-leer verifizieren, kann mit `-SimulateBackendCrash` Worker-Recovery samt Crash-Metadata verifizieren, kann mit `-OccupyDefaultPort` die dynamische Portwahl bei belegtem Default-Port verifizieren, kann mit `-SimulateBackendShutdown` kontrolliertes Worker-Beenden plus Supervisor-Recovery verifizieren, kann mit `-AttachExternalBackend` das Andocken an ein externes Python-Backend ohne Sidecar-Spawn verifizieren, kann mit `-SimulateBackendStartupTimeout` einen nicht-ready Worker und Supervisor-Ersatzstart pruefen, kann mit `-StabilityDurationSec ...` wiederholte Health-/State-Probes, Backend-Working-Set-Samples und normalisierte Tauri/Backend-CPU-Samples erfassen und mit `-MaxBackendWorkingSetGrowthMB` auf Speicherwachstum bzw. mit `-MaxIdleCpuPercent` auf durchschnittliche Idle-CPU failen, kann mit `-LegacyDataDir ... -VerifyLegacyDataMigration -SimulateUpgrade` Legacy-Datenmigration und Datenerhalt pruefen, kann mit `-VerifyUninstall` den stillen Uninstaller auf Artefaktentfernung und Datenerhalt pruefen und entfernt anschliessend die Testinstallation. |
...
| `.github/workflows/release-windows.yml` | Umgesetzt: manueller und Tag-basierter Windows-NSIS-Release-Build mit GitHub-Release-Publish auf `v*` Tags. Der Standard-Release-Workflow richtet MSYS2/UCRT64 ein, baut FFmpeg Profile B mit `scripts\ffmpeg\build_profile_b_msys2.ps1`, uebergibt den resultierenden `MediaToolsDir` plus `-ValidateSlimMediaTools`, fuehrt `-RunMediaPreparationSmoke` und `-RunRuntimeDependencyFootprint` aus und sammelt `media-preparation-smoke.json` sowie `runtime-dependency-footprint.json` mit den Release-Artefakten ein. |
...
| `src/web_api.py` | Teilweise umgesetzt: Version/Runtime im Health-Endpoint, Session-Token-Middleware, `/api/runtime/shutdown`, `/api/runtime/support-bundle`, `/api/runtime/frontend-ready` und Frontend-Static-File-Serving mit SPA-Fallback. Offen: Update-Endpunkte (`/api/update/*`), falls ein eigener Python-Updater wieder aktiviert wird. |
...
4. Nutzerdaten (`transcripts.db`, `settings.json`, `.env`, `downloads/`) bleiben bei Upgrade und Deinstallation erhalten.
...
| 4 | **FFmpeg-Bundling** | Entscheidung: `ffmpeg`/`ffprobe` bleiben gebuendelt; Profile B ersetzt den bisherigen Full-/Gyan-Pfad als Standard fuer GitHub-Release, FastLocal und direkte lokale Tauri-Builds nach bestandenem installiertem Real-Workflow-Smoke. Gyan Essentials bleibt ein expliziter Fallback/Rollback-Pfad. | Mittel: beeinflusst Setup-Groesse und Robustheit. |
