...
1. **Default-Installer = voller Standard-Build fuer Cloud-/Online-Workflows**:
   ...
   - Status 2026-06-09: Der Standard-Build-Pfad nutzt Gyan `release essentials` statt eines beliebigen PATH-/Chocolatey-Full-Builds. `scripts/prepare_gyan_ffmpeg_essentials.ps1` laedt `ffmpeg-release-essentials.zip`, verifiziert die `.sha256`, extrahiert den `bin`-Ordner und liefert ihn als `MediaToolsDir`. `Frontend/src-tauri/tauri.conf.json` ruft den Sidecar-Build mit `-UseGyanFfmpegEssentials -ValidateSlimMediaTools` auf; der GitHub-Release-Workflow bereitet dieselben Tools explizit vor und uebergibt `-MediaToolsDir`.
...
| 4 | **FFmpeg-Bundling** | Entscheidung: `ffmpeg`/`ffprobe` bleiben gebuendelt; Gyan Essentials ersetzt den bisherigen Full-Build als Standardquelle. Offen bleibt nur ein spaeterer eigener LGPL-/Minimal-Build. | Mittel: beeinflusst Setup-Groesse und Robustheit. |
