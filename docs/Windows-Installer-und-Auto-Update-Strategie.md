...
1. **Inno Setup**, per-user Install nach `%LocalAppData%\Scriber`.
   ...
   - Stabiler `AppId` im Format `{GUID}` fuer saubere Upgrade-Erkennung.
    ...
    - Status 2026-06-09: Ein neuer Standard-NSIS-Build mit `scripts\build_windows.ps1 -SkipChecks -SkipSmoke -ReuseSidecarIfUnchanged -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` ist erfolgreich durchgelaufen. Anschliessende `-FastLocalInstaller`-Realbuilds bestaetigten Frontend-Typecheck, Tauri/NSIS-Bundle, Media-Preparation-Smoke, Runtime-Dependency-Footprint und Size-Report im schnellen lokalen Modus. `release-metadata\size-report.json` meldet den Installer mit `186.41 MiB` unter dem `220 MiB` Budget und den Backend-Resource-Tree mit `515.56 MiB`; `runtime-dependency-footprint.json` meldet keine Budget-Failures, keine fehlenden Required Paths und keine disallowed Paths. `media-preparation-smoke.json` meldet `5/5` bestandene Media-Checks. Der zweite Fast-Local-Realbuild traf den Sidecar-Cache nach frischem Vite-Build und meldete `364592 ms` Gesamtzeit. Ein installierter Smoke gegen das aktuelle Setup bestand mit Frontend/WebView-ready, installierten Media-Tools, Support-Bundle-Redaction, striktem Silent-Uninstall und `528.43 MiB` installierter App-Groesse unter dem `560 MiB` Smoke-Budget. `scripts\smoke_frontend_browser.py` bestand zusaetzlich mit YouTube-Suche-/URL-Thumbnails, File-Drag-and-Drop, Debug-Clear und YouTube-Processing-Detailpfad.
...
3. Jede Release-PR enthaelt den Groessenvergleich zur Vorversion.
   - Status 2026-06-09: `scripts/create_release_size_report.py` erzeugt `size-report.json` mit Artefaktgroessen, Top-Dateien und optionaler installierter App-Groesse. `scripts/build_windows.ps1` ruft den Report nach `latest.json`/`SHA256SUMS.txt` auf und failt standardmaessig, wenn das groesste Installer-Artefakt ueber `-MaxInstallerSizeMB 220` liegt. `scripts/build_windows.ps1` schreibt zusaetzlich `release-metadata\build-timing.json`; der Sidecar-Build schreibt `sidecar-build-metadata.json`, das Cache-Status, Phasenzeiten, Media-Tools und PySide6-Pruning-Evidenz enthaelt. `scripts/smoke_windows_installer.ps1 -MaxInstalledSizeMB <MB>` misst den temporaeren Installationsordner und kann die installierte Groesse als hartes Gate pruefen. Der GitHub-Workflow kopiert `release-metadata\*.json`, sodass `size-report.json`, `build-timing.json`, `media-preparation-smoke.json` und `runtime-dependency-footprint.json` als Release-Artefakte verfuegbar sind. Der aktuelle Standard-Full-NSIS-Setup liegt mit `186.41 MiB` unter dem 220-MiB-Gate. Der Standard-Backend-Resource-Ordner liegt nach Pillow-AVIF-Pruning bei `515.56 MiB`; `ffmpeg.exe` und `ffprobe.exe` bleiben mit jeweils rund `133 MiB` die groessten Bloecke. Das installierte 450-MiB-Ziel darf nicht durch stilles Entfernen von ffmpeg/ffprobe, PySide6 oder ONNXRuntime aus dem Standard-Windows-Build erreicht werden; sinnvolle Folgearbeit ist ein kleineres kompatibles Media-Tools-Bundle oder weiterer PyInstaller-Dependency-Cleanup mit installierten Smokes.
