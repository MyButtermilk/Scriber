...
1. **Default-Installer = voller Standard-Build fuer Cloud-/Online-Workflows**:
   ...
   - Status 2026-06-09: Der direkte lokale Tauri-Build nutzt Gyan `release essentials` als konservativen Fallback statt eines beliebigen PATH-/Chocolatey-Full-Builds. `scripts/prepare_gyan_ffmpeg_essentials.ps1` laedt `ffmpeg-release-essentials.zip`, verifiziert die `.sha256`, extrahiert den `bin`-Ordner und liefert ihn als `MediaToolsDir`. `Frontend/src-tauri/tauri.conf.json` ruft den Sidecar-Build mit `-UseGyanFfmpegEssentials -ValidateSlimMediaTools` auf. Der GitHub-Release-Workflow baut dagegen Profile B ueber MSYS2/UCRT64 und uebergibt den erzeugten `MediaToolsDir`.
...
| 4 | **FFmpeg-Bundling** | Entscheidung: `ffmpeg`/`ffprobe` bleiben gebuendelt; Profile B ersetzt den bisherigen Full-Build als GitHub-Release-Quelle nach bestandenem installiertem Real-Workflow-Smoke. Gyan Essentials bleibt lokaler/direct-Tauri-Fallback. | Mittel: beeinflusst Setup-Groesse und Robustheit. |
