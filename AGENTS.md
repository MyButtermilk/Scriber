...
- `scripts/build_windows.ps1 -FastLocalInstaller` is the explicit fast local installer mode. It enables `-ReuseSidecarIfUnchanged`, skips full Python tests and the Tauri release smoke, keeps frontend typecheck/build, and forces media-preparation plus runtime-footprint reports with standard component budgets. `-SkipPythonTests` and `-SkipFrontendTypeCheck` are narrower check controls; `-SkipChecks` still skips both for compatibility. `-ReuseSidecarIfUnchanged` temporarily injects only the sidecar hash-cache mode into Tauri's `beforeBundleCommand`. The sidecar cache hashes normal input files by content (`length + sha256`) and reserves mtimes for tool metadata so unchanged frontend dist files do not invalidate the cache only because Vite rewrote timestamps. `-MediaToolsDir <path>` forwards an explicit media-tool candidate directory, usually together with `-ValidateSlimMediaTools`. `-PrunePySide6Translations`, `-PrunePySide6UnusedPlugins`, and `-PrunePySide6SoftwareOpenGl` are explicit PySide6 size experiments and must be paired with installed Live Mic overlay smokes before becoming release defaults.
...
- Installed app size reduction must preserve bundled media-tool functionality. The setup artifact is under the 220 MiB gate, but the current bundled backend resource tree is still dominated by full ffmpeg/ffprobe binaries. Use `-SkipBundledFfprobe` only for explicit comparison builds; do not make ffmpeg-only packaging the standard Windows build until media workflow smokes prove the reduced bundle.
