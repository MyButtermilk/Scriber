...
- The sidecar spec bundles the `yt-dlp` Python package and ONNXRuntime for Pipecat Silero VAD. `scripts/build_tauri_backend_sidecar.ps1 -BundleMediaTools` requires local `ffmpeg` and `ffprobe`, copies them into the sidecar output, and validates each copied binary with `-version`. `-ValidateSlimMediaTools` is the explicit gate for smaller FFmpeg candidates; it validates the Scriber-required WebM/Opus, MP3, AAC/Opus/MP3 decode, muxer, and common demuxer capabilities before packaging accepts the binary. `scripts/smoke_media_preparation.py` then exercises the real Python media-preparation helpers with the resolved tools: file-upload compression, upload audio extraction, YouTube post-download normalization, Azure-MAI MP3 preparation, and optional ffprobe duration probing. `scripts/build_windows.ps1 -RunInstallerMediaPreparationSmoke` runs that helper-path gate against the `backend\tools\ffmpeg` directory from the actually installed NSIS package. `-SkipBundledFfprobe` is an explicit size-experiment mode only; the standard Windows release path still bundles both tools. The pre-SciPy-removal 2026-06-03 comparison build reduced the installer from `205.79 MiB` to `163.43 MiB` and installed size to `481.10 MiB`, but it remains experimental until real provider/network media workflow smokes prove ffmpeg-only bundling.
...
- The frontend `/debug` route is the packaged-app debug console. It tails redacted backend/Tauri logs from `/api/runtime/logs`, supports text/source/level filters, color-coded severities, auto-refresh/auto-scroll, copy-visible logs, and support-bundle download.
...
- The current setup also passed `scripts\smoke_windows_installer.ps1 -VerifyFrontend -VerifyMediaPreparation -VerifySupportBundle -VerifyUninstall -MaxInstalledSizeMB 560` on 2026-06-09. The installed app measured `528.43 MiB`, frontend/WebView-ready was verified, installed media tools passed `5/5`, support-bundle token protection and redaction passed, and strict silent uninstall removed app artifacts while preserving the runtime-data sentinel.
