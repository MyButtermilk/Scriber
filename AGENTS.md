...
- `scripts/analyze_backend_runtime_dependencies.py` is the SciPy/ONNXRuntime sidecar footprint gate. It reports the frozen `_internal\scipy`, `_internal\scipy.libs`, and `_internal\onnxruntime` sizes, verifies required paths for `pyloudnorm`/`scipy.signal` and ONNXRuntime/Silero-VAD, and can enforce `--max-scipy-mb`, `--max-onnxruntime-mb`, and `--max-total-mb`. The current release-backend snapshot reports `107.46 MiB` total: `73.70 MiB` SciPy/SciPy libs and `33.76 MiB` ONNXRuntime. Do not remove SciPy subpackages or ONNXRuntime native files without passing this report plus the frozen runtime import check.
