...
- `.github/workflows/hybrid-pr-checks.yml` is the fast Windows PR gate. It runs focused Python hybrid gates, `npm run check`, `npm run build`, and `cargo test` on pull requests to `main`, pushes to `codex/hybrid-tauri-performance`, and manual dispatch. It intentionally does not build the NSIS installer or require ffmpeg/signing secrets.
