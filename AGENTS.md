...
- Azure MAI upload preparation is latency-first: existing `.mp3` files upload directly, every non-MP3 file input is transcoded to 64k mono MP3 through `libmp3lame`, and live PCM buffers are encoded to MP3 before upload. Do not restore WAV upload for Azure MAI unless a measured provider requirement overrides the latency tradeoff.
...
- The sidecar spec bundles the `yt-dlp` Python package and ONNXRuntime for Pipecat Silero VAD. GitHub Windows releases now build the custom FFmpeg Profile B media tools through MSYS2/UCRT64 and pass the produced `MediaToolsDir` into `scripts/build_windows.ps1`. Gyan Essentials remains the conservative local/direct-Tauri fallback: `scripts/prepare_gyan_ffmpeg_essentials.ps1` downloads Gyan `ffmpeg-release-essentials.zip`, verifies the published `.sha256`, extracts the `bin` directory, and returns it as `MediaToolsDir`. `Frontend/src-tauri/tauri.conf.json` still calls the sidecar build with `-UseGyanFfmpegEssentials -ValidateSlimMediaTools` for direct local `npm run tauri:build -- --bundles nsis` builds, so they do not copy an arbitrary PATH/Chocolatey full build by default. Explicit `-MediaToolsDir` overrides the Gyan download. `scripts/build_tauri_backend_sidecar.ps1 -BundleMediaTools` copies `ffmpeg` and `ffprobe` into the sidecar output and validates each copied binary with `-version`; `-ValidateSlimMediaTools` validates the Scriber-required WebM/Opus, MP3, stdout `pcm_s16le`, AAC/Opus/MP3/FLAC/ALAC decode, muxer, common demuxer including raw `s16le`, filter, and local `file`/`pipe` protocol capabilities before packaging accepts the binary. It also runs `scripts/ffmpeg/validate_ffmpeg_profile.py --profile B` and writes `tools\ffmpeg\ffmpeg-profile-manifest.json` with hashes, sizes, visible capabilities, build flags, and GPL/nonfree/network/excluded-feature warnings. FFmpeg parser requirements remain in `docs/FFmpeg-Footprint-Strategy.md`, but there is no portable `ffmpeg -parsers` list command; parser coverage is validated through configure/buildconf evidence plus media-smoke fixtures. `scripts/smoke_media_preparation.py` then exercises the real Python media-preparation helpers with the resolved tools: file-upload compression, upload audio extraction, YouTube post-download normalization, Azure-MAI MP3 preparation, and optional ffprobe duration probing. `scripts/build_windows.ps1 -RunInstallerMediaPreparationSmoke` runs that helper-path gate against the `backend\tools\ffmpeg` directory from the actually installed NSIS package. `scripts/build_windows.ps1 -RunInstallerRealMediaWorkflowSmoke` runs real installed file and YouTube transcription workflows and requires completed transcript plus summary output. `-SkipBundledFfprobe` is an explicit size-experiment mode only; the standard Windows release path still bundles both tools.
- `scripts/ffmpeg/create_profile_b_build_kit.py` is the reproducible Profile B custom-build helper. It writes `configure-profile-b.args`, `configure-profile-b.sh`, and `profile-b-build-plan.json` for a local-media-only FFmpeg build with MP3, WebM/Opus, stdout PCM, raw `s16le`, and `file`/`pipe` support, while excluding network protocols, GPL/nonfree flags, video encoders, and hardware stacks. The generated build kit is not itself a compiled binary; produced binaries still need the profile manifest, `scripts/ffmpeg/smoke_profile_b_fixtures.py`, media-preparation smoke, sidecar gate, installed-package smokes, and real workflow smoke before release use.
- `scripts/ffmpeg/build_profile_b_msys2.ps1` is the Windows compile runner for Profile B. It expects MSYS2/UCRT64, can install required packages with `pacman` when `-InstallDependencies` is passed, clones the configured FFmpeg source ref, runs the generated `configure-profile-b.sh`, copies the required adjacent UCRT64 runtime DLLs (`libmp3lame-0.dll`, `libopus-0.dll`, `libwinpthread-1.dll`) beside the produced tools, writes `profile-b-msys2-build-report.json`, and then runs manifest, fixture-smoke, and media-preparation gates. Use `-PlanOnly` first on machines where MSYS2 is not installed or before touching the build toolchain. A real 2026-06-09 MSYS2/UCRT64 Profile-B build passed with portable media tools at `4.98 MiB` total (`ffmpeg.exe`, `ffprobe.exe`, and three runtime DLLs), `26/26` fixture checks, `5/5` media-preparation checks, and the sidecar slim-media validation/copy gate. A full NSIS build with this explicit `MediaToolsDir` passed installed frontend and media-preparation smokes with installer `102.96 MiB`, installed app `267.28 MiB`, backend `254.42 MiB`, and installed `tools\ffmpeg` `5.84 MiB`. The real installed workflow smoke also passed `2/2`: a file workflow with Azure-MAI MP3 preparation and completed summary, plus a real YouTube URL workflow for `https://www.youtube.com/watch?v=0wEjbSYNUM8` with download/normalization/transcription/summary. The GitHub Windows release workflow now uses this Profile-B build as the release media-tool input.
...
- Tauri `beforeBundleCommand` runs the sidecar build with `-SkipFrontendBuild -InstallPyInstaller -BundleMediaTools -UseGyanFfmpegEssentials -ValidateSlimMediaTools -CopyToTauriRelease`, so direct local `npm run tauri:build -- --bundles nsis` can produce an installer with the backend resource included and the smaller validated Gyan Essentials fallback media tools. Use `scripts/build_windows.ps1 -MediaToolsDir <profile-b-bin> -ValidateSlimMediaTools` or the GitHub release workflow when the release Profile-B media-tool input is required.
...
- `scripts/check_backend_runtime_imports.py` is the sidecar preflight for startup imports such as local `pyloudnorm`, ONNXRuntime, Pipecat frames, Silero VAD, and `src.web_api`; `scripts/build_tauri_backend_sidecar.ps1` runs it before PyInstaller and again through the frozen `scriber-backend --runtime-import-check` so missing bundled runtime dependencies fail the build early.
- `scripts/analyze_backend_runtime_dependencies.py` is the frozen sidecar footprint gate. It expects `_internal\scipy` and `_internal\scipy.libs` to be absent, verifies required ONNXRuntime/Silero-VAD native paths, rejects ONNXRuntime sample/tool data and unused Pillow AVIF binaries, and reports standard release components for the complete backend, `_internal`, bundled `tools\ffmpeg`, PySide6, Google/gRPC, and Pillow. It can enforce `--max-scipy-mb`, `--max-onnxruntime-mb`, `--max-total-mb`, `--max-backend-mb`, `--max-internal-mb`, `--max-media-tools-mb`, `--max-pyside6-mb`, `--max-google-grpc-mb`, and `--max-pillow-mb`. The last Full-FFmpeg release-backend snapshot after Pillow AVIF pruning reported `33.75 MiB` for SciPy/ONNXRuntime dependency total (`0.00 MiB` SciPy/SciPy libs and `33.75 MiB` ONNXRuntime) and component sizes around `515.57 MiB` backend, `221.05 MiB` `_internal`, `267.01 MiB` media tools, `71.71 MiB` PySide6, `11.37 MiB` Google/gRPC, and `4.99 MiB` Pillow. After the Gyan Essentials switch, the measured Tauri release backend is `441.88 MiB`, `_internal` is `221.05 MiB`, media tools are `193.32 MiB`, PySide6 is `71.71 MiB`, Google/gRPC is `11.37 MiB`, Pillow is `4.99 MiB`, ONNXRuntime is `33.75 MiB`, and SciPy remains absent; standard budgets should use about `500 MiB` backend and `210 MiB` media tools. With the Profile-B candidate, the measured backend is `254.42 MiB`, `_internal` `221.06 MiB`, media tools `5.84 MiB`, PySide6 `71.71 MiB`, Google/gRPC `11.37 MiB`, Pillow `4.99 MiB`, ONNXRuntime `33.75 MiB`, and SciPy absent; candidate budgets used `325 MiB` backend and `10 MiB` media tools. Keep local `pyloudnorm` import-compatible with Pipecat; do not reintroduce SciPy, Pillow AVIF, or remove standard media/overlay/provider components without updating this gate and proving the size tradeoff.
...
- `scripts/create_release_metadata.py` writes `latest.json` and `SHA256SUMS.txt` for release artifacts. `scripts/create_release_size_report.py` writes `size-report.json`, gates the largest installer artifact with `-MaxInstallerSizeMB` from `scripts/build_windows.ps1` (default 220 MiB), and can include installed-app size/top-file data when an install directory is provided. `scripts\validate_windows_authenticode.ps1` validates Authenticode status, expected publisher, and optional timestamp after an external signing step; pass `-OutputPath` to persist the UTF-8-without-BOM JSON report consumed by the final readiness validator. `scripts/build_windows.ps1 -RequireAuthenticodeSignature` writes that report to `release-metadata\authenticode.json`. `scripts/validate_tauri_updater_metadata.py` validates the Tauri updater manifest shape, verifies local artifact size/checksum against `SHA256SUMS.txt` during Windows builds, and can require non-empty signatures. `scripts/prepare_tauri_updater_config.py` enables `createUpdaterArtifacts` and injects the updater public key/endpoints for signed release builds.
- `.github/workflows/hybrid-pr-checks.yml` is the fast Windows PR gate. It runs focused Python hybrid gates, including release-readiness, Authenticode/updater validators, Tauri stability wiring, and the media-preparation smoke script tests; then frontend `npm ci --no-audit --no-fund`, `npm run check`, `npm run build`, and Tauri Rust `cargo test` on pull requests to `main`, pushes to `codex/hybrid-tauri-performance`, and manual dispatch. The Rust job builds `Frontend/dist/public` before `cargo test` because Tauri's `frontendDist` contract is evaluated from a fresh checkout, and it creates an empty `Frontend/src-tauri/target/release/backend` resource directory so Tauri's configured backend resource path exists without building the real sidecar. It intentionally does not build the NSIS installer or require ffmpeg/signing secrets.
- `.github/workflows/release-windows.yml` builds the Windows NSIS artifact on `workflow_dispatch` or `v*` tags and uploads/publishes release files. It sets up MSYS2/UCRT64, builds FFmpeg Profile B through `scripts\ffmpeg\build_profile_b_msys2.ps1`, exports `SCRIBER_RELEASE_MEDIA_TOOLS_DIR`, and passes `-MediaToolsDir ... -ValidateSlimMediaTools` to `scripts\build_windows.ps1` so the release sidecar uses the custom Profile-B media tools instead of Chocolatey/PATH FFmpeg or Gyan. It runs `scripts\build_windows.ps1 -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` with explicit SciPy, ONNXRuntime, backend, `_internal`, media-tools, PySide6, Google/gRPC, and Pillow budgets so `release-metadata\media-preparation-smoke.json`, `release-metadata\runtime-dependency-footprint.json`, and `release-metadata\build-timing.json` are collected with the release artifacts. If `SCRIBER_REQUIRE_AUTHENTICODE_SIGNATURE=1`, it passes the Authenticode validation gate through `scripts/build_windows.ps1`; optional `SCRIBER_AUTHENTICODE_PUBLISHER` and `SCRIBER_REQUIRE_AUTHENTICODE_TIMESTAMP=1` make the gate stricter. If `SCRIBER_TAURI_UPDATER_PUBLIC_KEY` and `TAURI_SIGNING_PRIVATE_KEY` are present, the workflow enables Tauri updater artifacts and signature-required metadata validation. On `v*` tag releases with updater signing configured, the workflow runs `scripts\verify_tauri_updater_publication.py` after `softprops/action-gh-release` with retries and uploads `updater-publication.json` as CI publication evidence.
- The current sidecar spec is the standard cloud-provider build. It includes ONNXRuntime only for Silero VAD, uses the repository-local `pyloudnorm` compatibility package instead of SciPy for Pipecat loudness, and excludes heavy local ASR/tooling stacks (`onnx`, `numba`, `llvmlite`, `torch`, NeMo, ONNX-ASR, SciPy). Treat local ASR packaging as a separate optional package path.
...
- The frontend `/debug` route is the packaged-app debug console. It tails redacted backend/Tauri logs from `/api/runtime/logs`, supports text/source/level filters, color-coded severities, auto-refresh/auto-scroll, copy-visible logs, log-file clearing through token-protected `DELETE /api/runtime/logs`, and support-bundle download.
...
- `scripts/run_microphone_hardware_matrix.ps1` is the guided Windows runner for the full physical microphone matrix. It requires label substrings for USB, dock, Bluetooth, and favorite microphones, runs all eight scenario smokes with prompts, writes standardized `microphone-hardware-*.json` artifacts, and runs the final matrix validator unless `-SkipValidation` is passed. Use `-PlanOnly` to generate a redacted UTF-8-without-BOM operator plan without touching the backend; the plan reports `readyForPhysicalRun` and `missingLabelParameters` so operators can fix labels before starting physical device actions.
...
- `scripts/verify_tauri_updater_publication.py` fetches the published HTTPS updater `latest.json`, validates it with signature-required Tauri updater metadata rules, verifies that the final URL after redirects is still absolute HTTPS, compares the downloaded SHA256 with the local release metadata, and writes the publication report consumed by the final release-readiness validator.
- `scripts/validate_hybrid_release_readiness.py` is the final external-evidence aggregator for the hybrid goal. It requires a passing physical microphone matrix, signed absolute-HTTPS Tauri updater metadata with at least one named release artifact, a passing `media-preparation-smoke.json` from `scripts\smoke_media_preparation.py` or `scripts\build_windows.ps1 -RunMediaPreparationSmoke`, a passing `runtime-dependency-footprint.json` from `scripts\analyze_backend_runtime_dependencies.py` or `scripts\build_windows.ps1 -RunRuntimeDependencyFootprint`, a publication report for the fetched `latest.json` whose original URL and final redirect URL are absolute HTTPS, and an Authenticode validation JSON report whose artifact names include the release artifacts listed in `latest.json`. It should remain red for unsigned local builds and is the last check before claiming the hardware/signing/updater release gates are closed.
- `scripts/run_hybrid_release_readiness.ps1` is the top-level final release-readiness runner. It validates the physical microphone matrix, creates or reuses the media-preparation and runtime-dependency footprint reports, creates or reuses updater-publication and Authenticode reports, then runs `scripts\validate_hybrid_release_readiness.py` to write `hybrid-release-readiness.json`. Use `-PlanOnly` to inspect the exact commands and the structured `requiredEvidence` checklist without touching external endpoints or artifacts. The plan JSON is written as UTF-8 without BOM.
...
- A fresh standard NSIS build passed on 2026-06-09 before the Gyan Essentials switch through `scripts\build_windows.ps1 -SkipChecks -SkipSmoke -ReuseSidecarIfUnchanged -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` with explicit SciPy, ONNXRuntime, backend, `_internal`, media-tools, PySide6, Google/gRPC, and Pillow budgets. Subsequent `scripts\build_windows.ps1 -FastLocalInstaller` runs also passed with frontend typecheck, Tauri/NSIS bundle, media-preparation smoke, runtime dependency footprint, updater metadata validation, and release size report. That Full-FFmpeg setup artifact was `186.41 MiB` under the 220 MiB gate; the standard backend resource tree was `515.56 MiB`, `_internal` about `221.05 MiB`, media tools `267.01 MiB`, PySide6 `71.71 MiB`, Google/gRPC `11.37 MiB`, Pillow `4.99 MiB`, ONNXRuntime `33.75 MiB`, and SciPy absent. After the Gyan Essentials switch, `scripts\build_windows.ps1 -FastLocalInstaller -MaxBackendRuntimeDependencyMB 500 -MaxMediaToolsRuntimeDependencyMB 210 -MaxInstallerSizeMB 190` passed with `useGyanFfmpegEssentials=true`, SHA256 `6f58ce889f59c311410f7d2b18895b33c03456463486f3b1ebc93d97a0f54541`, sidecar cache hit, installer `152.98 MiB`, backend `441.88 MiB`, media tools `193.32 MiB`, release media-preparation smoke `5/5`, and runtime-footprint budgets backend `500 MiB` plus media tools `210 MiB`. Profile B then passed a complete Fast-Local NSIS build with explicit `MediaToolsDir`, installed frontend smoke, installed media-preparation smoke, and real installed File/YouTube workflow smoke: installer `102.96 MiB`, installed app `267.28 MiB`, backend `254.42 MiB`, media tools `5.84 MiB`, release media-preparation smoke `5/5`, real workflows `2/2`, and runtime-footprint budgets backend `325 MiB` plus media tools `10 MiB`.
- `release-metadata\media-preparation-smoke.json` passed `5/5` checks, `release-metadata\runtime-dependency-footprint.json` reported no budget failures, missing required paths, disallowed paths, or unexpected SciPy, and the latest `release-metadata\build-timing.json` reported `364592 ms` total build time for the second `-FastLocalInstaller` real run with `cacheHit=true` after a fresh Vite build. The embedded sidecar cache key was `71765fa4896f2a2d2e91f83afa9c2ee360494af3109cd9c253d679c86794a12d`; the sidecar phase took `51072 ms` without PyInstaller. A direct identical sidecar-only run with the same content-based cache key reported `40107 ms` without PyInstaller.
- The current Gyan Essentials setup passed `scripts\smoke_windows_installer.ps1 -VerifyFrontend -VerifyMediaPreparation -VerifySupportBundle -VerifyUninstall -MaxInstalledSizeMB 500` on 2026-06-09. The installed app measured `454.75 MiB`, frontend/WebView-ready was verified, installed media tools passed `5/5`, support-bundle token protection and redaction passed, and strict silent uninstall removed app artifacts while preserving the runtime-data sentinel.
- `scripts\smoke_frontend_browser.py --output tmp\frontend-browser-smoke-current.json` also passed on 2026-06-09 with `7` routes, no critical console/page errors, YouTube thumbnail interactions for search and URL lookup, file drag-and-drop, debug clear, transcript processing refresh, and token-required browser-state coverage.
- A prior explicit `-SkipBundledFfprobe -SkipChecks` comparison build produced a 163.43 MiB setup artifact, a 468.23 MiB backend resource tree, and a 481.10 MiB installed app; installed frontend/uninstall smoke and focused media-tool tests passed, but real YouTube/file/Azure-MAI media smokes are still required before making that mode a release default. The broader installed-package smoke set passed on 2026-06-02 with Tauri-origin frontend CORS, tokenized `/api/runtime`, real WebView readiness, cleanup, strict uninstall, support-bundle redaction, crash recovery, controlled worker shutdown recovery, startup-timeout recovery, default-port-conflict handling, and external-backend attach. Legacy Tkinter/Python tray paths are maintenance-only fallback per `docs/Legacy-Desktop-Fallback-Decision.md`; real release updates still require configured signing keys and a published signed updater manifest.
...
powershell -ExecutionPolicy Bypass -File scripts\build_windows.ps1 -RunMediaPreparationSmoke
powershell -ExecutionPolicy Bypass -File scripts\build_windows.ps1 -RunInstallerMediaPreparationSmoke
powershell -ExecutionPolicy Bypass -File scripts\build_windows.ps1 -RunRuntimeDependencyFootprint
...
powershell -ExecutionPolicy Bypass -File scripts\smoke_windows_installer.ps1 -VerifyMediaPreparation
...
powershell -ExecutionPolicy Bypass -File scripts\validate_windows_authenticode.ps1 -Path Frontend\src-tauri\target\release\scriber-desktop.exe Frontend\src-tauri\target\release\bundle\nsis\Scriber_0.1.0_x64-setup.exe -OutputPath tmp\hybrid-baseline\authenticode.json
python scripts\validate_hybrid_release_readiness.py --hardware-input-dir tmp\hybrid-baseline --updater-metadata Frontend\src-tauri\target\release\release-metadata\latest.json --media-preparation-report Frontend\src-tauri\target\release\release-metadata\media-preparation-smoke.json --runtime-dependency-footprint-report Frontend\src-tauri\target\release\release-metadata\runtime-dependency-footprint.json --updater-publication-report tmp\hybrid-baseline\updater-publication.json --authenticode-report tmp\hybrid-baseline\authenticode.json --output tmp\hybrid-baseline\hybrid-release-readiness.json
...
- Summaries: `SCRIBER_SUMMARIZATION_MODEL`, `SCRIBER_AUTO_SUMMARIZE`, `SCRIBER_SUMMARY_MIN_WORDS`, `SCRIBER_SUMMARY_MAX_WORDS`, `SCRIBER_SUMMARY_GEMINI_THINKING_BUDGET_TOKENS`, `SCRIBER_SUMMARY_GEMINI_MAX_TOKENS_RETRIES`, `SCRIBER_SUMMARY_GEMINI_RETRY_MAX_OUTPUT_TOKENS`
...
Current summarization default is `gemini-flash-latest`. Gemini summary requests set an explicit `thinkingConfig.thinkingBudget`, retry `MAX_TOKENS` with a larger `maxOutputTokens`, and discard repeated partial `MAX_TOKENS` responses instead of saving truncated summaries as successful output.
...
- Installed app size reduction must preserve bundled media-tool functionality. Full FFmpeg/ffprobe has been replaced by Profile B in the GitHub Windows release workflow after installed frontend/media and real File/YouTube workflow smokes passed. Gyan Essentials remains the conservative local/direct-Tauri fallback. Use `-SkipBundledFfprobe` only for explicit comparison builds.
