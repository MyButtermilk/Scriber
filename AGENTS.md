...
- Tauri `beforeBundleCommand` runs the sidecar build with `-SkipFrontendBuild -InstallPyInstaller -BundleMediaTools -UseProfileBFfmpeg -ValidateSlimMediaTools -ReuseSidecarIfUnchanged -CopyToTauriRelease`, so direct local `npm run tauri:build -- --bundles nsis` uses the same Profile-B media-tool standard and local sidecar cache as the fast installer path.
...
- `scripts/analyze_backend_runtime_dependencies.py` is the frozen sidecar footprint gate. It expects `_internal\scipy` and `_internal\scipy.libs` to be absent, verifies required ONNXRuntime/Silero-VAD native paths, rejects ONNXRuntime sample/tool data and unused Pillow AVIF binaries, and reports standard release components for the complete backend, `_internal`, bundled `tools\ffmpeg`, PySide6, Google/gRPC, and Pillow. It can enforce `--max-scipy-mb`, `--max-onnxruntime-mb`, `--max-total-mb`, `--max-backend-mb`, `--max-internal-mb`, `--max-media-tools-mb`, `--max-pyside6-mb`, `--max-google-grpc-mb`, and `--max-pillow-mb`. The Profile-B standard build uses budgets around `325 MiB` backend and `10 MiB` media tools; the measured Profile-B backend was `254.42 MiB`, `_internal` `221.06 MiB`, media tools `5.84 MiB`, PySide6 `71.71 MiB`, Google/gRPC `11.37 MiB`, Pillow `4.99 MiB`, ONNXRuntime `33.75 MiB`, and SciPy absent. Older Full-FFmpeg and Gyan measurements remain historical comparison points only. Keep local `pyloudnorm` import-compatible with Pipecat; do not reintroduce SciPy, Pillow AVIF, or remove standard media/overlay/provider components without updating this gate and proving the size tradeoff.
...
- `scripts/build_windows.ps1 -RunRuntimeDependencyFootprint` runs `scripts\analyze_backend_runtime_dependencies.py` against `target\release\backend` and writes `release-metadata\runtime-dependency-footprint.json`. `-MaxScipyRuntimeDependencyMB`, `-MaxOnnxRuntimeDependencyMB`, `-MaxPythonRuntimeDependencyMB`, `-MaxBackendRuntimeDependencyMB`, `-MaxInternalRuntimeDependencyMB`, `-MaxMediaToolsRuntimeDependencyMB`, `-MaxPySide6RuntimeDependencyMB`, `-MaxGoogleGrpcRuntimeDependencyMB`, and `-MaxPillowRuntimeDependencyMB` turn it into explicit size budgets. `-FastLocalInstaller` now defaults to Profile-B budgets when no explicit `-MediaToolsDir` is passed: `325 MiB` backend and `10 MiB` media tools.
- `scripts/build_windows.ps1 -FastLocalInstaller` is the explicit fast local installer mode. It enables `-UseProfileBFfmpeg`, `-ValidateSlimMediaTools`, and `-ReuseSidecarIfUnchanged` when no explicit `-MediaToolsDir` is passed, skips full Python tests and the Tauri release smoke, keeps frontend typecheck/build, and forces media-preparation plus runtime-footprint reports with standard Profile-B component budgets. `-SkipPythonTests` and `-SkipFrontendTypeCheck` are narrower check controls; `-SkipChecks` still skips both for compatibility. The sidecar cache hashes normal input files by content (`length + sha256`) and reserves mtimes for tool metadata so unchanged frontend dist files do not invalidate the cache only because Vite rewrote timestamps. `-MediaToolsDir <path>` forwards an explicit media-tool directory, usually together with `-ValidateSlimMediaTools`; the Profile-B path has been proven with `-MaxBackendRuntimeDependencyMB 325`, `-MaxMediaToolsRuntimeDependencyMB 10`, `-InstallerMaxInstalledSizeMB 360`, `-RunInstallerFrontendSmoke`, `-RunInstallerMediaPreparationSmoke`, and `-RunInstallerRealMediaWorkflowSmoke`. `-PrunePySide6Translations`, `-PrunePySide6UnusedPlugins`, and `-PrunePySide6SoftwareOpenGl` are explicit PySide6 size experiments and must be paired with installed Live Mic overlay smokes before becoming release defaults.
...
- A fresh standard NSIS build passed on 2026-06-09 before the Gyan Essentials switch through `scripts\build_windows.ps1 -SkipChecks -SkipSmoke -ReuseSidecarIfUnchanged -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` with explicit SciPy, ONNXRuntime, backend, `_internal`, media-tools, PySide6, Google/gRPC, and Pillow budgets. Those Full-FFmpeg and Gyan Essentials builds remain historical comparison points. Profile B then passed complete Fast-Local NSIS builds, installed frontend smoke, installed media-preparation smoke, and real installed File/YouTube workflow smoke: current installer `102.98 MiB`, installed app `267.28 MiB`, backend `254.42 MiB`, media tools `5.84 MiB`, release media-preparation smoke `5/5`, real workflows `2/2`, and runtime-footprint budgets backend `325 MiB` plus media tools `10 MiB`.
- `release-metadata\media-preparation-smoke.json` passed `5/5` checks, `release-metadata\runtime-dependency-footprint.json` reported no budget failures, missing required paths, disallowed paths, or unexpected SciPy, and the latest `release-metadata\build-timing.json` reported `538270 ms` total build time for a `-FastLocalInstaller` Profile-B standard run with `cacheHit=false` because the current frontend dist changed the content key. That run built and cached key `e360b748323c9c8b7311577c6df04a999bb73c541c1468fda9b6dd13f89552a8`; a direct identical sidecar-only run with `-UseProfileBFfmpeg -ValidateSlimMediaTools -ReuseSidecarIfUnchanged` then reported `cacheHit=true`, no PyInstaller phase, and `51429 ms` total sidecar duration.
- The historical Gyan Essentials fallback setup passed `scripts\smoke_windows_installer.ps1 -VerifyFrontend -VerifyMediaPreparation -VerifySupportBundle -VerifyUninstall -MaxInstalledSizeMB 500` on 2026-06-09. The installed app measured `454.75 MiB`, frontend/WebView-ready was verified, installed media tools passed `5/5`, support-bundle token protection and redaction passed, and strict silent uninstall removed app artifacts while preserving the runtime-data sentinel.
...
- Installed app size reduction must preserve bundled media-tool functionality. Full FFmpeg/ffprobe has been replaced by Profile B in the GitHub Windows release workflow, fast local installer path, and direct Tauri bundle path after installed frontend/media and real File/YouTube workflow smokes passed. Gyan Essentials remains an explicit fallback only. Use `-SkipBundledFfprobe` only for explicit comparison builds.
