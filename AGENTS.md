...
- The sidecar spec bundles the `yt-dlp` Python package and ONNXRuntime for Pipecat Silero VAD. `scripts/build_tauri_backend_sidecar.ps1 -BundleMediaTools` requires local `ffmpeg` and `ffprobe`, copies them into the sidecar output, and validates each copied binary with `-version`. `-SkipBundledFfprobe` is an explicit size-experiment mode only; the standard Windows release path still bundles both tools. The 2026-06-03 comparison build reduced the installer from `205.79 MiB` to `163.43 MiB` and installed size to `481.10 MiB`, but it remains experimental until real media workflow smokes prove ffmpeg-only bundling.
