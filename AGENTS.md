...
- `scripts/build_windows.ps1 -ReuseSidecarIfUnchanged` temporarily injects the sidecar hash-cache mode into Tauri's `beforeBundleCommand` for faster local installer builds. `-MediaToolsDir <path>` forwards an explicit media-tool candidate directory, usually together with `-ValidateSlimMediaTools`. `-PrunePySide6Translations`, `-PrunePySide6UnusedPlugins`, and `-PrunePySide6SoftwareOpenGl` are explicit PySide6 size experiments and must be paired with installed Live Mic overlay smokes before becoming release defaults.
...
- A fresh standard NSIS build passed on 2026-06-09 through `scripts\build_windows.ps1 -SkipChecks -SkipSmoke -ReuseSidecarIfUnchanged -RunMediaPreparationSmoke -RunRuntimeDependencyFootprint` with explicit SciPy, ONNXRuntime, backend, `_internal`, media-tools, PySide6, Google/gRPC, and Pillow budgets. The current setup artifact is `186.40 MiB` under the 220 MiB gate; the standard backend resource tree is `515.57 MiB`, `_internal` is `221.05 MiB`, media tools are `267.01 MiB`, PySide6 is `71.71 MiB`, Google/gRPC is `11.37 MiB`, Pillow is `4.99 MiB`, ONNXRuntime is `33.75 MiB`, and SciPy remains absent.
- `release-metadata\media-preparation-smoke.json` passed `5/5` checks, `release-metadata\runtime-dependency-footprint.json` reported no budget failures, missing required paths, disallowed paths, or unexpected SciPy, and `release-metadata\build-timing.json` reported `590451 ms` total build time with a clean Sidecar cache miss in that concrete NSIS run.
