...
- The sidecar spec bundles the `yt-dlp` Python package. `scripts/build_tauri_backend_sidecar.ps1 -BundleMediaTools` requires local `ffmpeg` and `ffprobe`, copies them into the sidecar output, and validates each copied binary with `-version`.
