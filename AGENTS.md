...
- `scripts/smoke_tauri_desktop.ps1 -VerifyFrontend` fetches the frontend entrypoint and referenced JS/CSS assets from the running backend static fallback. Use it when validating that the installed app starts with both backend and bundled frontend available.
