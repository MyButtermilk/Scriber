...
python scripts\validate_hybrid_release_readiness.py --hardware-input-dir tmp\hybrid-baseline --updater-metadata Frontend\src-tauri\target\release\release-metadata\latest.json --updater-publication-report path\to\updater-publication.json --authenticode-report path\to\authenticode.json --output tmp\hybrid-baseline\hybrid-release-readiness.json
```
