...
- Tauri `beforeBundleCommand` runs the sidecar build with `-SkipFrontendBuild -InstallPyInstaller -BundleMediaTools -UseGyanFfmpegEssentials -ValidateSlimMediaTools -CopyToTauriRelease`, so `npm run tauri:build -- --bundles nsis` can produce an installer with the backend resource included and the smaller validated Gyan Essentials media tools.
...
- `scripts/build_windows.ps1 -FastLocalInstaller` is the explicit fast local installer mode. It enables `-ReuseSidecarIfUnchanged`, skips full Python tests and the Tauri release smoke, keeps frontend typecheck/build, and forces media-preparation plus runtime-footprint reports with standard component budgets. `-SkipPythonTests` and `-SkipFrontendTypeCheck` are narrower check controls; `-SkipChecks` still skips both for compatibility. `-ReuseSidecarIfUnchanged` temporarily injects only the sidecar hash-cache mode into Tauri's `beforeBundleCommand`. The sidecar cache hashes normal input files by content (`length + sha256`) and reserves mtimes for tool metadata so unchanged frontend dist files do not invalidate the cache only because Vite rewrote timestamps. `-MediaToolsDir <path>` forwards an explicit media-tool candidate directory, usually together with `-ValidateSlimMediaTools`; the Profile-B candidate path has been proven with `-MaxBackendRuntimeDependencyMB 325`, `-MaxMediaToolsRuntimeDependencyMB 10`, `-InstallerMaxInstalledSizeMB 360`, `-RunInstallerFrontendSmoke`, and `-RunInstallerMediaPreparationSmoke`. `-PrunePySide6Translations`, `-PrunePySide6UnusedPlugins`, and `-PrunePySide6SoftwareOpenGl` are explicit PySide6 size experiments and must be paired with installed Live Mic overlay smokes before becoming release defaults.
...
- Installed app size reduction must preserve bundled media-tool functionality. Full FFmpeg/ffprobe has been replaced by Gyan Essentials in the conservative standard path, and the much smaller Profile-B candidate has passed installed frontend/media smokes. Use `-SkipBundledFfprobe` only for explicit comparison builds; do not make Profile B the standard Windows media-tool input until real installed YouTube/file workflow smokes prove the reduced bundle beyond synthetic media-preparation gates.
