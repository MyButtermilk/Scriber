...
const PROCESSING_MEETING_STATES = new Set<MeetingState>(["stopping", "finalizing", "analyzing"]);
...
function isOlderMeetingSummary(incoming: MeetingSummary, current: MeetingSummary | null | undefined): boolean {
  ...
  return (
    Number.isFinite(incomingUpdatedAt) && Number.isFinite(currentUpdatedAt) && incomingUpdatedAt < currentUpdatedAt
  );
}
...
export function applyMeetingProgressEvent(
  ...
  queryClient.setQueryData<MeetingDetail>(["/api/meetings", meetingId], (current) =>
    current
      ? {
          ...current,
          processingProgress: mergeMeetingProcessingProgress(current.processingProgress, incoming),
        }
      : current,
  );
}

export function applyMeetingSummaryEvent(queryClient: QueryClient, meeting: MeetingSummary): void {
  ...
  queryClient.setQueryData<MeetingsResponse>(MEETING_LIST_QUERY_KEY, (current) => {
    ...
    if (hasNewerMeetingSummary(meeting, [current.items.find((item) => item.id === meeting.id), current.activeMeeting]))
      return current;
    ...
    return {
      ...
      activeMeeting: activeMeeting ?? (current.activeMeeting?.id === meeting.id ? null : current.activeMeeting),
    };
  });
  queryClient.setQueryData<InfiniteData<MeetingsResponse, number>>(MEETING_HISTORY_QUERY_KEY, (current) => {
    ...
    if (
      hasNewerMeetingSummary(meeting, [
    ...
    const items =
      index >= 0 ? loaded.map((item, itemIndex) => (itemIndex === index ? meeting : item)) : [meeting, ...loaded];
    const total = Math.max(current.pages[0]?.total ?? 0, loaded.length) + (index >= 0 ? 0 : 1);
    let cursor = 0;
    const pages = current.pages.map((page) => {
      ...
      return {
        ...
        total,
        offset,
        activeMeeting: activeMeeting ?? (page.activeMeeting?.id === meeting.id ? null : page.activeMeeting),
      };
    });
    return {
      ...current,
...
export function applyMeetingSegmentEvent(queryClient: QueryClient, meetingId: string, segment: MeetingSegment): void {
  queryClient.setQueryData<MeetingDetail>(["/api/meetings", meetingId], (current) =>
    current
      ? {
          ...current,
...
export function applyMeetingCheckpointEvent(
  ...
  queryClient.setQueryData<MeetingDetail>(["/api/meetings", meetingId], (current) => {
    ...
    const transcriptCheckpoints =
      index >= 0
        ? current.transcriptCheckpoints.map((item, itemIndex) => (itemIndex === index ? checkpoint : item))
        : [...current.transcriptCheckpoints, checkpoint];
    transcriptCheckpoints.sort(
      (left, right) => left.commitOrdinal - right.commitOrdinal || left.sequence - right.sequence,
    );
...
export function applyMeetingActionItem(queryClient: QueryClient, meetingId: string, item: MeetingActionItem): void {
...
export function applyMeetingNoteEvent(queryClient: QueryClient, meetingId: string, note: MeetingNote): void {
...
export function applyMeetingSpeakerProfileSplit(queryClient: QueryClient, meetingId: string, speakerId: string): void {
  queryClient.setQueryData<MeetingSpeakerAssignmentsResponse>(
    ["/api/meetings", meetingId, "speaker-assignments"],
    (current) =>
      current
