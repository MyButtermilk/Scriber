...
test("Outlook sync time distinguishes today, yesterday, and older calendar data", () => {
  ...
  assert.match(formatOutlookSyncMoment(new Date(2026, 6, 15, 9, 5).toISOString(), now, "en-US") ?? "", /^today at /);
  assert.match(
    formatOutlookSyncMoment(new Date(2026, 6, 14, 18, 30).toISOString(), now, "en-US") ?? "",
    /^yesterday at /,
  );
  assert.match(formatOutlookSyncMoment(new Date(2026, 6, 10, 8, 0).toISOString(), now, "en-US") ?? "", /^Jul 10 at /);
});
