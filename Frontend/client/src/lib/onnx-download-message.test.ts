...
import { initializeLocaleCatalog, translate, type TranslationValues } from "@/i18n";
...
test("localizes every structured ONNX download progress message", () => {
  assert.equal(
    localizeOnnxDownloadMessage("Downloading model files (int8). This can take a while...", german, germanNumber),
  ...
  assert.equal(
    localizeOnnxDownloadMessage("Downloading encoder_model.onnx (12.5 MB/800.0 MB)", german, germanNumber),
...
test("preserves the English source forms and safely falls back for unknown messages", () => {
  ...
  assert.equal(localizeOnnxDownloadMessage("Waiting for cache lock", german, germanNumber), "Waiting for cache lock");
