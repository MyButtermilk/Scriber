...
export function SkeletonCard({ className, variant = "list" }: SkeletonCardProps) {
  if (variant === "grid") {
    return (
      <div className={cn("neu-skeleton-card rounded-xl p-4 bg-card", className)}>
        {/* Thumbnail */}
  ...
  return (
    <div className={cn("neu-skeleton-card rounded-xl p-4 bg-card flex items-center gap-4", className)}>
...
export function SkeletonList({ count = 3, variant = "list", className }: SkeletonListProps) {
  return (
    <div className={cn(variant === "grid" ? "grid grid-cols-2 gap-3" : "space-y-3", className)}>
