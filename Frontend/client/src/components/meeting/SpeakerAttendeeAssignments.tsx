...
export function SpeakerAttendeeAssignments({
  ...
  const mergeMutation = useMutation({
    mutationFn: async () => {
      if (!mergeTarget || !mergeSource) throw new Error("Choose two speaker identities to merge.");
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 marker:content-none">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold">Name detected speakers</p>
            <p className="mt-0.5 truncate text-xs text-muted-foreground">
              {assignmentsQuery.isLoading
                ? "Checking saved voices and participant details…"
                : unresolvedCount > 0
      ...
      <div className="border-t border-border/60 p-4">
        {assignmentsQuery.isLoading ? (
          ...
          <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-3" role="alert">
            ...
            <p className="mt-1 text-xs text-muted-foreground">The transcript and any linked Outlook event remain saved.</p>
              ...
              <div className="flex min-w-0 items-start gap-2.5">
                ...
                <div>
                  <p className="text-xs font-semibold">
                    {event ? "Saved voice and account matches run first on this device." : "Give each detected speaker a clear meeting name."}
                  </p>
                  <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                    {event
                      ? <>AI suggestions are optional. If you request them, Scriber sends short transcript excerpts and participant names to {assignmentsQuery.data?.llmModel || "your configured summary model"}. Outlook email addresses are not sent. Every suggestion stays unconfirmed until you approve it. You can also enter a person, team, or room name that stays only in this meeting.</>
                  ...
                  <div key={item.speakerId} className="grid gap-3 p-3.5 lg:grid-cols-[minmax(150px,0.72fr)_minmax(220px,1fr)_auto] lg:items-start">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {itemResolved && <Badge variant="outline" className="border-emerald-500/40 text-[10px] text-emerald-700 dark:text-emerald-300"><Check className="mr-1 h-3 w-3" />{item.confirmedCustomName ? "Meeting name" : "Confirmed"}</Badge>}
                      </div>
                    ...
                    <div className="min-w-0">
                      <Select value={selectedParticipantId} onValueChange={(participantId) => setSelectedParticipantIds((current) => ({ ...current, [item.speakerId]: participantId }))}>
                        <SelectTrigger className="h-10 min-w-0 bg-background" aria-label={`Name source for ${item.currentDisplayName || item.speakerLabel}`}>
                          <SelectValue placeholder={attendees.length > 0 ? "Choose participant or enter a name…" : "Enter a meeting name…"} />
                        </SelectTrigger>
                        <SelectContent>
                          ...
                          <SelectItem value={CUSTOM_NAME_VALUE}>Enter another name…</SelectItem>
                        </SelectContent>
                      </Select>
                      ...
                      {isCustomName && (
                        <div className="mt-2">
                          <label htmlFor={customNameInputId} className="text-[11px] font-medium text-foreground">Speaker name</label>
                          <Input
                            id={customNameInputId}
                            value={customName}
                            maxLength={120}
                            placeholder="For example: Project team or Berlin room"
                    ...
                    <div className="flex flex-wrap gap-2 lg:justify-end">
                      {itemResolved && (
                        <Button type="button" size="sm" variant="ghost" disabled={pendingThisSpeaker} onClick={() => confirmMutation.mutate({ kind: "participant", speakerId: item.speakerId, participantId: null, suggestionSource: "manual" })}>Remove</Button>
                      )}
                      <Button
                        type="button"
                        size="sm"
                      ...
                      >
                        ...
                        {isCustomName ? "Save name" : itemResolved ? "Update" : "Confirm"}
                      </Button>
                    </div>
                  </div>
                );
            ...
            {mergeOptions.length >= 2 && (
              <div className="rounded-xl border border-border/65 bg-background/40 p-3.5">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-semibold">Merge duplicate speaker identities</p>
                    <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
                  <Select value={mergeTargetProfileId} onValueChange={(profileId) => {
                    ...
                    <SelectTrigger className="h-9 min-w-0 bg-background text-xs" aria-label="Speaker identity to keep">
                      <SelectValue placeholder="Keep speaker…" />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <Select value={mergeSourceProfileId} onValueChange={setMergeSourceProfileId}>
                    <SelectTrigger className="h-9 min-w-0 bg-background text-xs" aria-label="Duplicate speaker identity to merge">
                      <SelectValue placeholder="Merge duplicate…" />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  >
                    <GitMerge className="mr-1.5 h-3.5 w-3.5" />Merge speakers
                  </Button>
                </div>
                {mergeMutation.isError && (
                  <p className="mt-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive" role="alert">
                    The speaker identities could not be merged. Nothing was changed.
                  </p>
                )}
              </div>
            )}
      ...
      <AlertDialog open={mergeConfirmationOpen} onOpenChange={setMergeConfirmationOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Merge these speaker identities permanently?</AlertDialogTitle>
            <AlertDialogDescription>
              {mergeTarget && mergeSource
                ? <>
                    {mergeDirectionChanged && <>To preserve the only saved Voice Library name, Scriber adjusted the merge direction. </>}
                    Scriber will keep <strong>{mergeTarget.displayName}</strong> and merge <strong>{mergeSource.displayName}</strong> into it. Their saved voice samples and future matches become one identity. This cannot be undone automatically.
                  </>
                : "Choose the speaker identity to keep and the duplicate to merge."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          {mergeMutation.isError && (
            <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive" role="alert">
              <p className="font-medium">The speaker identities could not be merged.</p>
              <p className="mt-1 text-xs">{mergeMutation.error.message || "Nothing was changed. Try again."}</p>
            </div>
          )}
          <AlertDialogFooter>
            ...
            >
              ...
              Keep {mergeTarget?.displayName || "speaker"} and merge
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
