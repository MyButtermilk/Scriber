...
async function fetchAssignments(meetingId: string, signal?: AbortSignal): Promise<MeetingSpeakerAssignmentsResponse> {
  const response = await fetchWithTimeout(
    apiUrl(`/api/meetings/${meetingId}/speaker-assignments`),
    {
      credentials: "include",
      signal,
    },
...
function attendeeOptions(event: OutlookCalendarEvent): OutlookCalendarContact[] {
  ...
  return [event.organizer, ...event.participants, event.currentUser ?? null].filter(
    (contact): contact is OutlookCalendarContact => {
...
function bestSuggestion(item: MeetingSpeakerAssignment): MeetingSpeakerSuggestion | null {
  return (
    [...item.suggestions].sort((left, right) => {
...
export function SpeakerAttendeeAssignments({
  ...
  const unresolvedCount =
    assignmentsQuery.data?.items.filter((item) => !item.confirmedAttendee && !item.confirmedCustomName).length ?? 0;
    ...
    setSelectedParticipantIds((current) => {
      ...
      for (const item of items) {
        ...
        if (item.confirmedCustomName) {
          ...
          next[item.speakerId] = initialMeetingParticipantId(attendees, "", item.confirmedAttendee, suggestedAttendee);
        } else if (currentId === CUSTOM_NAME_VALUE) {
          next[item.speakerId] = CUSTOM_NAME_VALUE;
        } else {
          next[item.speakerId] =
            initialMeetingParticipantId(attendees, currentId, null, suggestedAttendee) ||
  ...
  useEffect(() => {
    if (
      !focusRequest ||
      !(assignmentsQuery.data?.items ?? []).some((item) => item.speakerId === focusRequest.speakerId)
    )
      return;
    ...
    const focusFrame = window.requestAnimationFrame(() => {
      nestedFrame = window.requestAnimationFrame(() => {
        ...
        const row = rows
          ? Array.from(rows).find((candidate) => candidate.dataset.speakerAssignmentId === focusRequest.speakerId)
          : null;
        if (!row) return;
        ...
        row.querySelector<HTMLButtonElement>("[data-speaker-assignment-trigger]")?.focus({ preventScroll: true });
        clearHighlightTimer = window.setTimeout(() => {
          setHighlightedSpeakerId((current) => (current === focusRequest.speakerId ? "" : current));
        }, 2_400);
      });
    });
  ...
  const confirmMutation = useMutation({
    mutationFn: async (variables: ConfirmAssignmentVariables) => {
      ...
      const response = await apiRequest(
        "PATCH",
        `/api/meetings/${meetingId}/speakers/${variables.speakerId}/attendee`,
        body,
      );
            ...
            <p className="mt-0.5 truncate text-xs text-muted-foreground">
              {assignmentsQuery.isLoading
                ...
                : unresolvedCount > 0
                  ? t(
                      unresolvedCount === 1
                        ? "{{count}} speaker needs your confirmation"
                        : "{{count}} speakers need your confirmation",
        ...
        {assignmentsQuery.isLoading ? (
          ...
          <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-3" role="alert">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {t("The transcript and any linked Outlook event remain saved.")}
            </p>
            <Button
              type="button"
              size="sm"
          ...
          <p className="rounded-xl border border-dashed border-border/70 px-4 py-6 text-center text-sm text-muted-foreground">
            {t("No separate speakers were detected in this transcript.")}
          </p>
        ) : (
          <div className="space-y-3">
              ...
              <div className="flex min-w-0 items-start gap-2.5">
                ...
                <div>
                  <p className="text-xs font-semibold">
                    {event
                      ? t("Saved voice and account matches run first on this device.")
                      : t("Give each detected speaker a clear meeting name.")}
                  </p>
                  <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                    {event
                      ? t(
                          "AI suggestions are optional. If you request them, Scriber sends short transcript excerpts and participant names to {{model}}. Outlook email addresses are not sent. Every suggestion stays unconfirmed until you approve it. You can also enter a person, team, or room name that stays only in this meeting.",
                          { model: assignmentsQuery.data?.llmModel || t("your configured summary model") },
                        )
                      : t(
                          "Enter a person, team, room, or shared-microphone name. It stays only in this meeting and does not change the Voice Library or create an email recipient.",
                        )}
                  </p>
                </div>
              </div>
            ...
            {suggestMutation.isError && (
              <p
                ...
                role="alert"
              >
                {t("AI suggestions could not be created. You can still choose participants yourself.")}
              </p>
            )}
            ...
            <div className="divide-y divide-border/60 rounded-xl border border-border/65 bg-background/40">
              {assignmentsQuery.data?.items.map((item) => {
                ...
                const selectedAttendee =
                  attendees.find((attendee) => meetingContactId(attendee) === selectedParticipantId) ?? null;
                ...
                const pendingThisSpeaker =
                  confirmMutation.isPending && confirmMutation.variables?.speakerId === item.speakerId;
                ...
                const speakerName = rawSpeakerName === "Meeting audio" ? t("Meeting audio") : rawSpeakerName;
                return (
                  <div
                    key={item.speakerId}
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {itemResolved && (
                          <Badge
                            variant="outline"
                            className="border-emerald-500/40 text-ui-micro text-emerald-700 dark:text-emerald-300"
                          >
                            <Check className="mr-1 h-3 w-3" />
                            {item.confirmedCustomName ? t("Meeting name") : t("Confirmed")}
                          </Badge>
                        )}
                      </div>
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        {item.sourceHint === "microphone" ? t("Your microphone") : t("Meeting audio")}
                      </p>
                      <Button
                        type="button"
                        size="sm"
                        ...
                        title={
                          playableSpeakerIds.has(item.speakerId)
                      ...
                      {!itemResolved && item.profileMatch && (
                        <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                          {t("Local voice profile")} · {item.profileMatch.displayName}
                          {confidenceLabel(item.profileMatch.confidence, formatNumber, t)
                            ? ` · ${confidenceLabel(item.profileMatch.confidence, formatNumber, t)}`
                            : ""}
                          {item.profileMatch.evidenceCount
                            ? ` · ${t(item.profileMatch.evidenceCount === 1 ? "{{count}} saved sample" : "{{count}} saved samples", { count: formatNumber(item.profileMatch.evidenceCount) })}`
                            : ""}
                      ...
                      {!itemResolved && suggested && (
                        <div className="mt-2">
                          <p className="text-ui-micro font-semibold uppercase tracking-[0.1em] text-primary">
                            {suggestionLabel(suggested.source, t)}
                          </p>
                          <p className="mt-0.5 text-[11px] leading-4 text-muted-foreground">
                            {suggested.attendee.name || suggested.attendee.address}
                            {confidenceLabel(suggested.confidence, formatNumber, t)
                              ? ` · ${confidenceLabel(suggested.confidence, formatNumber, t)}`
                              : ""}
                    ...
                    <div className="min-w-0">
                      <Select
                        ...
                        onValueChange={(participantId) =>
                          setSelectedParticipantIds((current) => ({ ...current, [item.speakerId]: participantId }))
                        }
                      >
                        <SelectTrigger
                          data-speaker-assignment-trigger
                          className="h-10 min-w-0 bg-background"
                          aria-label={t("Name source for {{speaker}}", { speaker: speakerName })}
                        >
                          <SelectValue
                            placeholder={
                              attendees.length > 0
                                ? t("Choose participant or enter a name…")
                                : t("Enter a meeting name…")
                            }
                          />
                        </SelectTrigger>
                        <SelectContent>
                          {attendees.map((attendee) => {
                            ...
                            return (
                              <SelectItem key={attendeeId} value={attendeeId}>
                                ...
                                {attendeeQualifier(attendee, t)}
                                {suggestion ? ` · ${suggestionLabel(suggestion.source, t)}` : ""}
                              </SelectItem>
                            );
                          })}
                      ...
                      {selectedAttendee && (
                        <p className="mt-1.5 flex min-w-0 items-center gap-1.5 truncate text-[11px] text-muted-foreground">
                      ...
                      {isCustomName && (
                        <div className="mt-2">
                          <label htmlFor={customNameInputId} className="text-[11px] font-medium text-foreground">
                            {t("Speaker name")}
                          </label>
                          <Input
                          ...
                          <p className="mt-1.5 text-[11px] leading-4 text-muted-foreground">
                            {t(
                              "Only this meeting changes. Voice profiles, Outlook contacts, and email recipients stay untouched.",
                            )}
                          </p>
                        </div>
                      )}
              ...
              <div className="rounded-xl border border-border/65 bg-background/40 p-3.5">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] leading-4 text-muted-foreground">
                      {t(
                        "If two detected speakers are the same voice, keep the correct identity and merge the duplicate into it. Saved samples and future voice matches are combined permanently. If only one profile has a saved name, Scriber always preserves it.",
                      )}
                    </p>
                  </div>
                </div>
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  ...
                  >
                    <SelectTrigger
                      className="h-9 min-w-0 bg-background text-xs"
                      aria-label={t("Speaker identity to keep")}
                    >
                    ...
                    <SelectContent>
                      {mergeOptions.map((option) => (
                        <SelectItem key={option.profileId} value={option.profileId}>
                          {option.displayName}
                          {option.speakerLabel && option.speakerLabel !== option.displayName
                            ? ` · ${option.speakerLabel}`
                            : ""}
                        </SelectItem>
                      ))}
                  ...
                  <Select value={mergeSourceProfileId} onValueChange={setMergeSourceProfileId}>
                    <SelectTrigger
                      className="h-9 min-w-0 bg-background text-xs"
                      aria-label={t("Duplicate speaker identity to merge")}
                    >
                      <SelectValue placeholder={t("Merge duplicate…")} />
                    </SelectTrigger>
                    <SelectContent>
                      {mergeOptions
                        .filter((option) => option.profileId !== mergeTargetProfileId)
                        .map((option) => (
                          <SelectItem key={option.profileId} value={option.profileId}>
                            {option.displayName}
                            {option.speakerLabel && option.speakerLabel !== option.displayName
            ...
            {confirmMutation.isError && (
              <p
                ...
                role="alert"
              >
                {t("The speaker assignment was not saved. Try again.")}
              </p>
            )}
            <p className="flex items-start gap-2 text-[11px] leading-4 text-muted-foreground">
              <Mail className="mt-0.5 h-3.5 w-3.5 shrink-0" />
              {t(
                "Confirmed mappings improve speaker names in the transcript. Email drafts use only suitable participants from the linked Outlook event—never a freely entered speaker name—and always show recipients for review.",
              )}
            </p>
          </div>
        )}
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {mergeTarget && mergeSource ? (
                <>
                  {mergeDirectionChanged && (
                    <>{t("To preserve the only saved Voice Library name, Scriber adjusted the merge direction.")} </>
                  )}
                  {t(
                    "Scriber will keep {{target}} and merge {{source}} into it. Their saved voice samples and future matches become one identity. This cannot be undone automatically.",
                    { target: mergeTarget.displayName, source: mergeSource.displayName },
                  )}
                </>
              ) : (
