...
function ContactLine({ name, address, label }: { name: string; address: string; label?: string }) {
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-start gap-3 py-1.5 text-xs">
      ...
      {label && (
        <span className="rounded-full border border-border/70 px-2 py-0.5 text-ui-micro font-medium text-muted-foreground">
          {label}
        </span>
      )}
    </div>
...
export function OutlookMeetingPicker({
  ...
  const showSavedCalendarWarning = Boolean(status?.connected && events && (status.lastError || eventsError));
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 px-4 py-3.5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 id="outlook-meeting-picker-title" className="text-sm font-semibold">
              {t("Today in Outlook")}
            </h3>
            <p className="mt-0.5 text-xs leading-5 text-muted-foreground">
              {events?.account?.address
                ? t("Pick the calendar event for {{account}}.", {
                    account: events.account.name
                      ? `${events.account.name} (${events.account.address})`
                      : events.account.address,
                  })
      ...
      {showSavedCalendarWarning && (
        <div
          className="mx-3 mt-3 flex items-start gap-2.5 rounded-xl border border-amber-300/60 bg-amber-500/10 px-3 py-2.5 text-amber-950 dark:text-amber-100"
          role="alert"
        >
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[11px] leading-4">
              {lastSyncMoment
                ? t(
                    "Scriber is showing saved meetings from {{moment}}. Changes made in Outlook since then may be missing.",
      ...
      {statusLoading ? (
        ...
        <div className="p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("Restart Scriber or try the connection again in Settings.")}
          </p>
        ...
        <div className="p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("Open Meeting settings for simple setup help.")}
          </p>
          <Button type="button" size="sm" variant="outline" className="mt-3" onClick={onOpenSettings}>
            {t("Open settings")}
          </Button>
        </div>
        ...
        <div className="p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("Today's events stay hidden until Microsoft confirms which account is connected.")}
          </p>
          <Button type="button" size="sm" variant="outline" className="mt-3" onClick={onOpenSettings}>
            {t("View connection status")}
          </Button>
        </div>
        ...
        <div className="p-4" role="alert">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("Microsoft needs you to sign in again. Your saved meetings remain unchanged.")}
          </p>
          <Button type="button" size="sm" variant="outline" className="mt-3" onClick={onOpenSettings}>
            {t("Reconnect in Settings")}
          </Button>
        </div>
      ) : !status.connected ? (
        <div className="p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("Scriber requests read-only calendar access and never receives your Microsoft password.")}
          </p>
          <Button type="button" size="sm" variant="outline" className="mt-3" onClick={onOpenSettings}>
            {t("Connect in Settings")}
          </Button>
        </div>
      ) : eventsLoading ? (
        <div className="flex min-h-24 items-center justify-center p-4">
        ...
        <div className="p-4" role="alert">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("Your existing meeting setup is unchanged. Refresh the calendar to try again.")}
          </p>
          <Button type="button" size="sm" variant="outline" className="mt-3" disabled={refreshing} onClick={onRefresh}>
            {t("Refresh calendar")}
          </Button>
        </div>
        ...
        <div className="p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t("You can still start an unlinked meeting and enter its title yourself.")}
          </p>
        </div>
      ) : (
        <div className="p-3">
          <div
            ...
            role="list"
            aria-label={t("Today's Outlook meetings")}
          >
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-sm font-semibold">
                        {event.subject || t("Untitled Outlook meeting")}
                      </span>
                      <span className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
                        <span className="inline-flex items-center gap-1">
                          <Clock3 className="h-3.5 w-3.5" />
                          {event.isAllDay ? t("All day") : formatEventTime(event.start_at, event.end_at, formatDate, t)}
                        </span>
                        <span className="inline-flex items-center gap-1">
                          <Users className="h-3.5 w-3.5" />
                          {t("{{count}} people", { count: formatNumber(eventContactCount(event)) })}
                        </span>
                      </span>
                    </span>
                  </button>
                    ...
                    <div className="border-t border-border/60 px-3 pb-3 pt-2.5">
                      ...
                      <div className="mt-1 max-h-48 divide-y divide-border/50 overflow-y-auto pr-1">
                        {event.organizer && (
                          <ContactLine
                            ...
                            label={event.organizer.isCurrentUser ? t("You · organizer") : t("Organizer")}
                          />
                        )}
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-border/50 pt-2.5 text-xs text-muted-foreground">
                          ...
                          {event.join_url && (
                            <button
                              ...
                              onClick={() => void openMeetingUrl(event.join_url)}
                            >
                              <ExternalLink className="h-3.5 w-3.5" />
                              {t("Open online meeting")}
                            </button>
                          )}
                        </div>
                      )}
                      {event.participants.length === 0 && !event.organizer && (
                        <p className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground">
                          ...
                          {t("No participant addresses are stored for this event.")}
                        </p>
                      )}
                    </div>
                  )}
          ...
          {events?.truncated && (
            <p className="px-1 pt-2 text-[11px] leading-4 text-muted-foreground">
              {t(
                "Only the first calendar entries for today are shown. Refresh after Outlook changes to get the latest list.",
              )}
            </p>
          )}
          {!selectedEvent && (
            <p className="px-1 pt-2 text-[11px] leading-4 text-muted-foreground">
              {t("Select an event to copy its title and keep its participants with the meeting.")}
            </p>
          )}
        </div>
      )}
