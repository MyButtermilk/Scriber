...
export function ActiveMeetingPill() {
  ...
  useEffect(() => {
    ...
    const enteredRecording = meetingState === "recording" && previousMeetingStateRef.current !== "recording";
  ...
  useEffect(() => {
    if (isMeetingWebSocketReconnect(wsHasConnectedRef.current, wsWasConnectedRef.current, isConnected)) {
  ...
  const staleSource =
    ...
    (now - lastFrameAtRef.current.microphone > 5_000 || now - lastFrameAtRef.current.system > 5_000);
  ...
  const status =
    meeting.state === "starting"
      ...
      : processing
        ? meeting.state === "analyzing"
          ? t("Building meeting brief")
          : t("Finalizing durable audio")
  ...
  return (
    ...
    >
      <div
        className={`mx-auto flex min-h-11 max-w-4xl items-center gap-3 rounded-xl border px-3 py-2 shadow-sm ${needsAttention ? "border-amber-300/70 bg-amber-500/10" : "border-border/70 bg-card/90"}`}
      >
        <button
          type="button"
        ...
        >
          <span
            className={`h-2.5 w-2.5 shrink-0 rounded-full ${needsAttention ? "bg-amber-500" : meeting.state === "starting" || meeting.state === "paused" || processing ? "bg-primary" : "bg-red-500"}`}
            aria-hidden="true"
          />
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-semibold">{meeting.title}</span>
            <span
              className="mt-0.5 flex items-center gap-2 truncate text-[11px] text-muted-foreground"
              aria-live="polite"
            >
              <span>{status}</span>
              {!processing && meeting.state !== "starting" && (
                <>
          ...
          <div className="flex shrink-0 items-center gap-1">
            {controls.pause && (
              <Button
                ...
                size="icon"
                variant="ghost"
                className="h-9 w-9 active:scale-[0.97]"
                disabled={controlMutation.isPending}
                onClick={() => controlMutation.mutate("pause")}
                aria-label={t("Pause active meeting")}
                data-testid="active-meeting-pause"
              >
            ...
            {controls.resume && (
              <Button
                ...
                aria-label={t("Resume active meeting")}
                data-testid="active-meeting-resume"
              >
            ...
            {controls.stop && (
              <Button
                ...
                size="icon"
                variant="ghost"
                className="h-9 w-9 text-destructive hover:text-destructive active:scale-[0.97]"
                disabled={controlMutation.isPending}
                onClick={() => controlMutation.mutate("stop")}
                aria-label={t("Stop active meeting")}
                data-testid="active-meeting-stop"
              >
