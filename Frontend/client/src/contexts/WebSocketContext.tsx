...
export type ScriberWebSocketMessage =
  ...
  | (BaseWsMessage & {
      ...
      meetingId: string;
      source: string;
      status: "reconnecting" | "recovered" | "degraded";
      reconnectCount: number;
    })
  | (BaseWsMessage & {
      type: "meeting_finalize_progress" | "meeting_analysis_progress";
      meetingId: string;
      progress: number;
      status: string;
    })
...
export function WebSocketProvider({
  ...
  const connect = useCallback(() => {
    ...
    const scheduleReconnect = () => {
      ...
      const delay = Math.min(reconnectDelay * Math.pow(1.5, reconnectCountRef.current), 30000);
