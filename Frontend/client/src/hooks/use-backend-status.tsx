...
function isManagedBackendStarting(status: TauriBackendStatus): boolean {
  ...
  return message.includes("starting") || message.includes("process started") || message.includes("restarting");
}
...
export function BackendStatusProvider({ children }: { children: ReactNode }) {
  ...
  const runHealthCheck = useCallback(async (): Promise<boolean> => {
    ...
    try {
      if (isTauriRuntime()) {
        try {
          await withDeadline(loadBackendBaseUrlFromTauri(), TAURI_ACCESS_TIMEOUT_MS, "Tauri backend access lookup");
        } catch (accessError) {
        ...
        if (!isTauriRuntime() && !backendSessionToken) {
          ...
          try {
            ...
            if (authProbe.status === 401) {
              ...
              setIsOnline(false);
              setError(
                "This backend requires a Scriber desktop session token. Open Scriber from the installed desktop app.",
              );
              setLastChecked(new Date());
              return false;
            }
