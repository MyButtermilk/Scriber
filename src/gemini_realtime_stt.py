...
class GeminiTranscribeLiveSTTService(FrameProcessor):
    ...
    async def _handle_response(self, raw: str) -> None:
        ...
        if interim_text:
            ...
            if self._drain_is_inferred and not _transcription_match_score(self._drain_interim_text or "", interim_text):
                # An unrelated hypothesis is new unfinished speech. A late
                # repeat/update of the carried hypothesis remains ambiguous:
                # arrival order alone cannot turn it into a terminal failure.
                # Both paths retain the full final deadline.
                self._drain_is_inferred = False
    ...
    async def _wait_for_final_drain(self, generation: int) -> bool:
        ...
        while self._final_generation <= generation:
            # generationComplete describes model output, not a correlated
            # input-transcription turn. Even shared-prefix hypotheses can be
            # distinct speech; never close their drain on that marker.
        ...
        while pending_generation is not None or observe_late_transcription:
            ...
            if not await self._wait_for_final_drain(pending_generation):
                if self._drain_is_inferred:
                    ...
                    # received. Without correlation, retain the full provider
                    # deadline even if generationComplete was received.
                    # Speculation alone is not a terminal provider failure.
