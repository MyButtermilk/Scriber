...
media-preparation smokes: installer `102.98 MiB`, temporary installed app
...
and summary output. The GitHub Windows release workflow, `-FastLocalInstaller`,
and direct local Tauri bundle path now use Profile B by default; Gyan Essentials
remains available only as an explicit fallback path.
...
After the Gyan Essentials fallback comparison, the local Fast-Local installer
measured `152.98 MiB` with backend `441.88 MiB` and media tools `193.32 MiB`.
The Profile-B build with MP3 support measured `102.98 MiB` for the installer,
...
installed package, so this is the current standard media-tool target.
