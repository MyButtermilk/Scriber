...
`scripts\validate_hybrid_release_readiness.py` is the final external-evidence aggregator for the hybrid goal. It combines the completed physical microphone matrix, signed HTTPS Tauri updater metadata plus optional local artifact/SHA256 verification, a publication report for the fetched `latest.json`, and the JSON output from `scripts\validate_windows_authenticode.ps1`. It is expected to fail on local unsigned builds or before real hardware/signing/publication evidence exists.
