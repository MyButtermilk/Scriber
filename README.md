...
reports the exact `scipy`, `scipy.libs`, and `onnxruntime` footprint, validates
the required frozen paths for `pyloudnorm`/`scipy.signal` startup and
ONNXRuntime/Silero-VAD, and can enforce `--max-scipy-mb`,
`--max-onnxruntime-mb`, and `--max-total-mb` budgets. On the current packaged
backend snapshot it reports `107.46 MiB` total: `73.70 MiB` for SciPy/SciPy
libs and `33.76 MiB` for ONNXRuntime. `scripts\build_windows.ps1
