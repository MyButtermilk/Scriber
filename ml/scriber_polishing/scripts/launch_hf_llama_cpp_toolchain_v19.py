"""Acquire one CAS claim and launch exactly one verified v19 CPU HF Job.

The claim is deliberately permanent.  Any failed or ambiguous submission must
be diagnosed from that claim and the HF job list; this command never retries.
"""
...
from scriber_polishing.hf_launch_gate import (  # noqa: E402
    ...
    HF_CLI_PREFIX,
    TOOLCHAIN_V19_CLAIM_PATH,
    HfLaunchGateError,
    bind_toolchain_v19_claim_to_argv,
    build_authoritative_campaign_inventory,
    build_toolchain_v19_claim,
    ...
    validate_preclaim_remote_state,
...
def _authoritative_all_jobs(api: HfApi) -> list[dict[str, object]]:
    return reconcile_authoritative_hf_job_snapshots(
        list(api.list_jobs(namespace="Buttermilk03")),
        _all_jobs(),
    )
...
def _claim_exists(api: HfApi, repository_id: str) -> bool:
    if not api.repo_exists(repository_id, repo_type=CLAIM_REPOSITORY_TYPE):
        return False
    return TOOLCHAIN_V19_CLAIM_PATH in api.list_repo_files(
        repository_id,
...
def _acquire_claim(
    ...
    *,
    ...
    info = api.repo_info(repository_id, repo_type=CLAIM_REPOSITORY_TYPE)
    if info.private is not True or not info.sha:
        raise HfLaunchGateError("launch-claim repository is not private or has no parent SHA")
    require_claim_never_existed(
        api,
        repository_id=repository_id,
        repository_type=CLAIM_REPOSITORY_TYPE,
        claim_path=TOOLCHAIN_V19_CLAIM_PATH,
    ...
    try:
        commit = api.create_commit(
            ...
            commit_message="Acquire exclusive Scriber toolchain v19 launch claim",
            operations=[
                CommitOperationAdd(
                    path_in_repo=TOOLCHAIN_V19_CLAIM_PATH,
        ...
        raise HfLaunchGateError(
            "exclusive v19 CAS claim failed; refusing submission"
        ) from error
    oid = str(commit.oid)
    if len(oid) != 40 or any(character not in "0123456789abcdef" for character in oid):
        raise HfLaunchGateError("toolchain claim commit OID is invalid")
    try:
        downloaded = Path(
            hf_hub_download(
                repo_id=repository_id,
                filename=TOOLCHAIN_V19_CLAIM_PATH,
                repo_type=CLAIM_REPOSITORY_TYPE,
                revision=oid,
                force_download=True,
            )
        ...
        if downloaded.is_symlink() or not downloaded.is_file() or downloaded.read_bytes() != payload:
            raise HfLaunchGateError("toolchain claim readback bytes differ")
    except HfLaunchGateError:
        raise
    except Exception as error:
        raise HfLaunchGateError("toolchain claim commit could not be read back exactly") from error
    return oid
...
def _write_receipt(path: Path, value: dict[str, object]) -> None:
    ...
    try:
        ...
        raise HfLaunchGateError(f"launch receipt already exists: {path}") from error
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = _output_entries()
    existing_claim = _claim_exists(api, CLAIM_REPOSITORY_ID)
    inventory = build_authoritative_campaign_inventory(
        authoritative_jobs,
        observed_at_utc=now,
        forbidden_attempt="v19",
        accounted_campaign_job_ids=contract.get("accounted_campaign_job_ids", []),
    )
    checked = validate_preclaim_remote_state(
        contract=contract,
        active_jobs=authoritative_jobs,
        output_entries=output,
        claim_path_exists=existing_claim,
        now_utc=now,
    )
    claim, owner = build_toolchain_v19_claim(
        checked,
        claimed_at_utc=now,
        campaign_inventory=inventory,
    )

    # Renew every remote gate immediately before the irreversible CAS claim.
    renewed_jobs = _authoritative_all_jobs(api)
    validate_preclaim_remote_state(
        contract=checked,
        active_jobs=renewed_jobs,
        output_entries=_output_entries(),
        claim_path_exists=_claim_exists(api, CLAIM_REPOSITORY_ID),
    ...
    if renewed_inventory["inventory_sha256"] != inventory["inventory_sha256"]:
        raise HfLaunchGateError(
            "authoritative campaign inventory changed before the v19 claim"
        )
    require_clean_checkout_at_head(
        checked.get("source_git_head"),
        repository_root=repository_root,
    )
    claim_commit = _acquire_claim(
        api,
    ...
    # The claim serializes compliant launchers.  A fresh global job/output gate
    # still catches non-compliant external activity before submission.
    ...
    if postclaim_inventory["inventory_sha256"] != inventory["inventory_sha256"]:
        raise HfLaunchGateError(
            "authoritative campaign inventory changed after the v19 claim"
        )
    if _output_entries():
        raise HfLaunchGateError(
            "v19 output changed after claim; claim retained and submission blocked"
        )
    launch_argv = bind_toolchain_v19_claim_to_argv(checked, owner)
    require_clean_checkout_at_head(
        checked.get("source_git_head"),
        repository_root=repository_root,
    )
    ...
    if completed.returncode != 0:
        raise HfLaunchGateError(
            "HF submission failed or was ambiguous; permanent claim retained, never retry: "
            + combined[-2000:]
        )
    ...
    receipt = {
        "schema_version": 1,
        "kind": "scriber_hf_toolchain_v19_launch_receipt",
