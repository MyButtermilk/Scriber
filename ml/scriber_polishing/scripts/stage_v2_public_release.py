#!/usr/bin/env python3
"""Stage the exact public V2 Q8_0/BF16 payload without uploading it."""

from __future__ import annotations

import argparse
import json
import sys
from collections.abc import Mapping
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(PROJECT_ROOT / "src"))

from scriber_polishing.v2_q8_bf16_release import (  # noqa: E402
    V2ReleaseError,
    canonical_json_bytes,
    stage_v2_public_bundle,
)


def _load_canonical_object(path: Path, label: str) -> dict[str, object]:
    source = path.expanduser().resolve()
    if source.is_symlink() or not source.is_file():
        raise V2ReleaseError(f"{label} must be a regular file: {source}")
    payload = source.read_bytes()
    try:
        value = json.loads(payload.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise V2ReleaseError(f"{label} is not UTF-8 JSON") from error
    if not isinstance(value, Mapping):
        raise V2ReleaseError(f"{label} must be a JSON object")
    materialized = dict(value)
    if payload != canonical_json_bytes(materialized):
        raise V2ReleaseError(f"{label} bytes are not canonical")
    return materialized


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--plan", type=Path, required=True)
    parser.add_argument("--quantization-result", type=Path, required=True)
    parser.add_argument("--quantization-root", type=Path, required=True)
    parser.add_argument("--gemma-license", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, required=True)
    args = parser.parse_args(argv)
    try:
        inventory = stage_v2_public_bundle(
            _load_canonical_object(args.plan, "V2 release plan"),
            quantization_result=_load_canonical_object(
                args.quantization_result, "V2 quantization result"
            ),
            quantization_root=args.quantization_root,
            gemma_license=args.gemma_license,
            output_root=args.output_root,
        )
    except (OSError, V2ReleaseError, ValueError) as error:
        print(f"v2_public_bundle=failed error={error}", file=sys.stderr)
        return 2
    print(
        "v2_public_bundle=staged "
        f"repository={inventory['repository_id']} variants=Q8_0,BF16 "
        "upload_performed=false"
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
