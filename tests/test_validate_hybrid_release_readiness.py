...
def write_rust_audio_app_prewarm_report(
    ...
    *,
    ...
    payload = {
        ...
        "summary": {
            "callbackCount": callback_count,
            "captureCycleCount": capture_cycles,
        ...
        json.dumps(
            {
                ...
                "checks": [
                    ...
                    {
                        "name": "rustMidSessionClean",
                        "ok": rust_mid_session_clean_ok,
                        "details": {
                            "checkedRustFramePipeSamples": samples,
                            "failureSampleCount": 0 if rust_mid_session_clean_ok else samples,
                        },
                    },
                    {
                        "name": "rustFramePipeFlow",
                        "ok": rust_frame_pipe_flow_ok,
                        "details": {
                            "checkedRustFramePipeSamples": samples,
                    ...
                    {
                        "name": "rustActiveCaptureStable",
                        "ok": rust_active_capture_stable_ok,
                        "details": {
                            "checkedRustFramePipeSamples": samples,
                            "unstableSampleCount": 0 if rust_active_capture_stable_ok else samples,
                        },
                    },
                    {
                        "name": "rustAlwaysOnMic",
                        "ok": rust_always_on_ok,
                        "details": {
                            "snapshotCount": 4,
...
def write_installed_live_recording_smoke_report(
    ...
    *,
    ...
    active_capture_mid_session_failure_reason: str = "",
    active_capture_frame_pipe_reader_end_reason: str = "running",
    active_capture_last_rust_mid_session_failure_reason: str = "",
        ...
        if include_audio_diagnostics:
            sample["audioDiagnostics"] = {
                ...
                "activeCapture": {
                    ...
                    "lastRustAudioMidSessionFailureReason": active_capture_last_rust_mid_session_failure_reason,
                    "midSessionFailureReason": active_capture_mid_session_failure_reason,
                    "sourceMidSessionFailureReason": active_capture_mid_session_failure_reason,
                    "framePipeReaderEndReason": active_capture_frame_pipe_reader_end_reason,
                    "sourceFramePipeReaderEndReason": active_capture_frame_pipe_reader_end_reason,
                },
...
def test_validate_release_readiness_rejects_installed_rust_live_recording_with_mid_session_failure(tmp_path: Path) -> None:
    ...
    write_installed_live_recording_smoke_report(
        ...
        rust_audio_available=True,
        frame_source="rust-frame-pipe",
        active_capture_mid_session_failure_reason="pipeClosed",
        active_capture_frame_pipe_reader_end_reason="pipeClosed",
        active_capture_last_rust_mid_session_failure_reason="pipeClosed",
    )
    ...
    failures = "\n".join(live_check["failures"])
    rust_evidence = live_check["details"]["rustAudioEvidence"]
    assert rust_evidence["midSessionFailureSampleCount"] == rust_evidence["sampleCount"]
    assert "activeCapture.lastRustAudioMidSessionFailureReason must be empty" in failures
...
def test_validate_release_readiness_rejects_recording_hot_path_mid_session_failure(tmp_path: Path) -> None:
    ...
    assert "recording hot-path comparison check failed: rustMidSessionClean" in comparison_check["failures"]
...
def test_validate_release_readiness_rejects_unstable_recording_hot_path_active_capture(tmp_path: Path) -> None:
    ...
    assert "recording hot-path comparison check failed: rustActiveCaptureStable" in comparison_check["failures"]
...
def test_validate_release_readiness_rejects_stale_recording_hot_path_comparison_without_active_capture_stable_check(tmp_path: Path) -> None:
    ...
    assert "recording hot-path comparison is missing check: rustActiveCaptureStable" in comparison_check["failures"]
...
def test_validate_release_readiness_accepts_required_long_rust_audio_app_prewarm_smoke(tmp_path: Path) -> None:
    ...
    assert app_check["details"]["minCaptureCycles"] == 2
    assert app_check["details"]["cycleCount"] == 2
