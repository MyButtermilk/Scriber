...
def test_summary_html_sanitizer_removes_executable_markup_and_attributes():
    ...
    assert normalize_summary_html(raw) == (
        "<section><h2>Title</h2><p>Hello <strong>world</strong></p>badmaillocalgood</section>"
    )


def test_summary_html_links_are_reduced_to_noninteractive_text():
    assert (
        normalize_summary_html('<p>Source: <a href="https://example.com" title="remote">example.com</a></p>')
...
def test_summary_document_repairs_markdown_drift_with_real_lead_structure():
    ...
    assert normalize_summary_document_html(markdown) == (
        "<section><h2>Overview</h2><p>Short standfirst.</p><ul><li>First fact</li><li>Second fact</li></ul></section>"
    )
...
def test_summary_document_drops_reasoning_leak_and_plain_preface():
    ...
    assert (
        normalize_summary_document_html(f"<think>We need to plan the answer first.</think>{structured}") == structured
    )
...
def test_summary_document_extracts_one_html_fence_with_surrounding_chatter():
    ...
    assert (
        normalize_summary_document_html(f"Here is the result:\n```html\n{structured}\n```\nHope this helps.")
        == structured
    )
...
def test_summary_document_rejects_unbalanced_or_misnested_markup():
    assert normalize_summary_document_html("<section><h2>Overview</h2><p>Short standfirst.</p><ul><li>Truncated") == ""
    assert normalize_summary_document_html("<section><h2>Overview</h2><p>Short standfirst.</section></p>") == ""
...
def test_summary_document_rejects_multiple_fenced_blocks():
    ...
    assert normalize_summary_document_html(f"```html\n{first}\n```\n```html\n{second}\n```") == ""
