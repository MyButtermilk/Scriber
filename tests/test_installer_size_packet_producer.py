    ...
    probe.write_text(
        "\n".join(
            (
                ...
                "    $null = 1 + 1",
                "}",
                "$evidenceBoundFalseStatus = Invoke-CapturedCommand -LogPath (Join-Path $LogRoot 'evidence-bound-false-status.log') -PowerShellScript -Command {",
                "    [System.IO.File]::WriteAllText((Join-Path $LogRoot 'evidence.json'), '{\"ok\":true}')",
                "    & $Python -c 'import sys;sys.exit(11)'",
                "}",
                ...
                "    evidenceBoundFalseStatus = [int]$evidenceBoundFalseStatus.exitCode",
