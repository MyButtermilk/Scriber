...
def _catalog(payloads: dict[str, bytes]) -> ModelCatalog:
    variants = {}
    prompt_template = "Fixture task\n\nTranskript:\n${transcript}\n\nBereinigte Fassung:\n\n"
    for variant in ("qad_q4_0",):
        ...
        payloads[policy_path] = json.dumps(
            {
                "schema_version": 2,
        ...
        variants[variant] = VariantSpec(
            ...
            prompt_template_sha256=_sha(prompt_template.encode()),
            generation_max_new_tokens=384,
        )
    return ModelCatalog(
        schema_version=3,
...
async def test_local_polishing_routes_are_token_protected_and_do_not_expose_credentials(
    ...
    try:
        responses = [
            ...
            await client.post("/api/local-polishing/models/qad_q4_0/install"),
            await client.delete("/api/local-polishing/model-operations/unknown"),
            await client.delete("/api/local-polishing/models/qad_q4_0"),
        ]
        ...
        assert {item["variant"] for item in public_payload["models"]} == {"qad_q4_0"}
...
async def test_install_is_idempotent_anonymous_and_broadcasts_authoritative_progress(
    ...
    try:
        ...
        first = await client.post("/api/local-polishing/models/qad_q4_0/install", headers=headers)
        second = await client.post("/api/local-polishing/models/qad_q4_0/install", headers=headers)
        ...
        assert progress_event["variant"] == "qad_q4_0"
        ...
        qad = next(item for item in status_payload["models"] if item["variant"] == "qad_q4_0")
        assert qad["status"] == "ready"
        assert qad["installed"] is True
...
async def test_install_operation_can_be_cancelled_without_starting_a_duplicate(
    ...
    await client.start_server()
    try:
        install = await client.post("/api/local-polishing/models/qad_q4_0/install")
...
async def test_uninstalled_model_cannot_be_selected_and_selected_model_cannot_be_deleted(
    ...
    monkeypatch.setenv("SCRIBER_LOCAL_POLISHING_VARIANT", "qad_q4_0")
    ...
    try:
        rejected = await client.put(
            "/api/settings",
            json={"postProcessingEngine": "local", "localPolishingVariant": "qad_q4_0"},
        )
        ...
        active_delete = await client.delete("/api/local-polishing/models/qad_q4_0")
        ...
        removed = await client.delete("/api/local-polishing/models/qad_q4_0")
        assert removed.status == 200
        state = manager.state().to_dict()
        qad = next(item for item in state["models"] if item["variant"] == "qad_q4_0")
        assert qad["installed"] is False
    finally:
        await _close(client, controller)


@pytest.mark.asyncio
async def test_sole_qad_variant_rejects_retired_precision_selection(
    monkeypatch,
    tmp_path,
):
    monkeypatch.setenv("SCRIBER_DATA_DIR", str(tmp_path / "data"))
    ...
    try:
        ...
        qad_selection = await client.put(
            "/api/settings",
            json={"postProcessingEngine": "local", "localPolishingVariant": "qad_q4_0"},
        )
        assert qad_selection.status == 200
        for _ in range(50):
            if manager.state().current_variant == "qad_q4_0":
                break
            await asyncio.sleep(0.01)
        assert manager.state().current_variant == "qad_q4_0"

        legacy_selection = await client.put(
            "/api/settings",
            json={"localPolishingVariant": "bf16"},
        )
        assert legacy_selection.status == 400
        assert manager.state().current_variant == "qad_q4_0"
        assert Config.LOCAL_POLISHING_VARIANT == "qad_q4_0"
    finally:
        await _close(client, controller)
...
async def test_local_selection_waits_for_verified_runtime_before_saving(
    ...
    update_task = asyncio.create_task(
        controller.update_settings({"postProcessingEngine": "local", "localPolishingVariant": "qad_q4_0"})
    )
...
async def test_local_selection_fails_closed_when_runtime_cannot_start(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match="verified local polishing runtime"):
        await controller.update_settings({"postProcessingEngine": "local", "localPolishingVariant": "qad_q4_0"})
...
@pytest.mark.asyncio
async def test_duplicate_qad_prewarm_is_coalesced_and_retired_variants_are_ignored(monkeypatch, tmp_path):
    class Snapshot:
        def to_dict(self):
            return {
                "available": True,
                "message": None,
                "currentVariant": None,
                "models": [{"variant": "qad_q4_0", "status": "ready", "installed": True}],
            }
    ...
    controller._schedule_local_polishing_prewarm("qad_q4_0")
    assert await asyncio.wait_for(polisher.started.get(), timeout=1.0) == (0, "qad_q4_0")
    controller._schedule_local_polishing_prewarm("qad_q4_0")
    controller._schedule_local_polishing_prewarm("bf16")
    await asyncio.sleep(0)
    assert polisher.started.empty()
    assert polisher.cancelled.empty()

    polisher.gates[0].set()
    active = controller._local_polishing_prewarm_tasks["qad_q4_0"]
...
async def test_live_controller_passes_local_engine_and_keeps_raw_fallback(
    ...
    assert diagnostic["model"] == "local:qad_q4_0"
